...
`free` follows ownership, and ownership follows the binding that received the allocation. A parameter borrows: the callee may read and write through it, but `free` on a parameter is refused with `cannot free a borrowed pointer; only its owner frees it`, and `move` on one is refused the same way, so a callee can never end a lifetime its caller still holds. A value read out of a container borrows too: `vec_get` hands back the stored pointer as a view of the container's backing, so freeing or moving that result is refused, and there is no owning removal that takes an element out of a container along with the right to free it.

One consequence is stated plainly rather than hidden: a linked structure whose nodes hang behind enum payloads or a pointer container cannot be deep freed by hand today. The function that walks it holds every node through a borrow, a match payload binder that is itself a pointer borrows the same way, and only the single binding that received each `alloc` could free that node. A payload string is the one exception, since `free` of a string payload releases the string's own heap bytes. For a tree that must reclaim per node, use the collected heap: a `collector<T>` owns its cells and reclaims unreachable ones without any `free` at all, which is exactly the shape a parser result or a long lived graph wants. Data that lives for the whole process can simply never free, which leaks nothing the process end does not reclaim.
