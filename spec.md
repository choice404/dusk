...
The marker rides beside the standard library's name and shape blessing of `vec_take` and `map_take`, the two recognized as owning at once, which is what lets a user's own `owning func` bless in the same release the standard takes still bless by name. The standard takes gain the marker and the name table retires in 1.9.1, closing the last of the by name blessing.
