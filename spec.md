...
The freeze closed with the bootstrap itself: 0.9.4 reached the fixpoint, the compiler written in dusk building itself to a byte identical result three stages deep, and 1.0.0 declares that compiler canonical with no language surface change of its own. The 0.5.4 surface 1.0.0 carried forward is unchanged start to finish across the whole line that held the freeze. 1.1.0 is the first release to add to the surface since: a `char`, a `char[N]`, and a `char[]` are now printable text, a string literal initializes a `char[N]` directly, and a `for` loop and a range slice both treat a string's bytes with the same discipline described in [Strings](#strings), [Arrays and Slices](#arrays-and-slices), and the [Builtins](#builtins) chapter. 1.2.0 keeps adding to it: `&&` and `||` now short circuit instead of evaluating both operands unconditionally, `==` and `!=` on a string compare content instead of pointer identity while comparison closes on a named list of comparable types, `+` concatenates two strings, a width cast converts an integer explicitly, `break` and `continue` are new statement keywords, and a runtime fault now names the source line that raised it. Each is a genuine change to what a program can write and what it means, recorded in full in [Expressions and Operators](#expressions-and-operators), [Strings](#strings), the [Builtins](#builtins) chapter, and [Memory Management](#memory-management). 1.4.0 opens the foreign boundary further: a `foreign` block may declare a variadic C function, a `@link` directive names a library for the linker and a `@csource` directive compiles a C file in alongside the runtime, and `!=` between two floats is corrected to the unordered comparison IEEE 754 defines. 1.4.1 through 1.4.3 open it the rest of the way: a C plain struct crosses by value as a `foreign` parameter or return, a capture free lambda or a named function crosses as a C callback, and `export "C"` with `dusk build --lib` compiles a dusk module into a static archive and a generated header any C ABI language links. Recorded in full in [Foreign Functions](#foreign-functions), [Linking and C Sources](#linking-and-c-sources), and [Comparison](#comparison). 1.5.0 widens the cast: `rune`, `float32`, and `float64` join the cast builtins and a cast now crosses between the integer and float families, a float to an integer saturating rather than leaving an out of range input undefined, recorded in [Numeric Casts](#numeric-casts). 1.5.1 adds the `hash` builtin, a deterministic 64-bit hash of a hashable scalar or string, the groundwork for a generic keyed map, recorded in [Hash](#hash). 1.5.2 delivers that map: `std.map` is generic over its key, `Map<K, V>`, K any hashable type. 1.5.3 rounds out `std.string` with the everyday manipulation set, a library change with no new surface. 1.6.0 adds the block comment: `/* */` nests, evaporates before parsing, and a comment containing a newline terminates a statement the way the newline itself does, recorded in [Comments](#comments), which also writes down the line comment the language has carried since the beginning. The same release teaches `dusk ir` a second target: `ir --target=wasm32 <file>` cross-emits the module's LLVM IR for wasm32-wasip1, the form a wasi toolchain links, with the native triple staying the default everywhere else; alongside it `std.os`'s errno read is renamed `os_errno`, recorded in [The errno convention](#the-errno-convention). 1.6.1 adds the doc comment and the `doc` command, recorded in [Doc Comments](#doc-comments). 1.7.0 changes no surface a correct program relies on: it closes an escape hole where a frame view buried in a call's returned aggregate crossed the frame boundary unseen, turns a misplaced type declaration into one recovering error instead of a cascade, lets `@param` name a generic type parameter and a doc comment bind to an `impl` head, and writes down rules long enforced, the interface-as-generic-argument rejection, the top level declaration rule, and the ownership boundary that sends a reclaiming tree to the collected heap. 1.7.1 gives the compiler a machine face, `check --json` reporting diagnostics as one deterministic JSON document with file local byte spans, and span keys joining the doc model, recorded in [The Machine Face](#the-machine-face). 1.8.0 gives dusk owning removal from a container: `vec_take` and `map_take` take an element out and hand the caller the owner, the checker's borrow net around containers settles into one model across every spelling, and a program can now hand write the deep free of a heap tree, recorded in [Memory Management](#memory-management). 1.8.1 is a standard library release with no new surface: `std.flags` parses a command line as a register then parse builder, `std.set` is a set over the generic map, `std.vector` gains `vec_map` and `vec_filter`, `std.time` gains `weekday` and a strict `parse_iso8601` that inverts `format_iso8601`, and `std.json` ships the hand written deep free of a parse tree as `json_free`, the first in-tree caller of the owning takes 1.8.0 introduced, recorded in [Imports and Standard Library](#imports-and-standard-library) and [Memory Management](#memory-management). 1.9.0 generalizes the owning take into a declared marker: an `owning func` head declares that its call result is the caller's own value, the checker blesses that result exactly as it blesses `vec_take`, and a `match` used as an expression now types its result and judges the ownership of its arms with one voice, recorded in [Memory Management](#memory-management). The same release closes a batch of checker gaps that previously reached `clang` or the generation backstop: a ground `free` of a non pointer, a `match` arm that yields a lambda, a pattern or constructor naming a variant its enum lacks, and an argument of the wrong type spelled as a field access are each now a named check error, and `dusk doc` binds a doc comment to a `foreign` function head, recorded in [Doc Comments](#doc-comments), [Sum Types (Enums)](#sum-types-enums), and [Functions](#functions). 1.10.0 makes block scoping and shadowing sound the whole way down: a binding introduced inside a nested block shadows an outer name only to the end of that block, the outer name is visible again after it with its own value and type, a lambda captures the bindings visible where it is written, and a `defer` resolves the names it reads in the function frame it was registered in rather than in whatever block is open at the return, recorded in [Blocks, Scopes, and Shadowing](#blocks-scopes-and-shadowing) and [Scope Cleanup with `defer`](#scope-cleanup-with-defer). The same release reserves the ambient allocator's name, refusing a binding that shadows a `using` allocator parameter since `alloc` and `free` in that body resolve through it, recorded in [`alloc` and `free` Are Sugar Over the In Scope Allocator](#alloc-and-free-are-sugar-over-the-in-scope-allocator). 1.10.1 rides the fixed scoping to finish the match expression: an arm body that ends in a bare `match` now value-izes into the enclosing match, so a nested match is the arm's value with no name to bind it to, every arm of a match expression must yield a value or diverge, and an inferred bind of a match records the ownership its arms agreed on so a borrowed result is refused a free even without an annotation, recorded in [Memory Management](#memory-management). 1.11.0 lets `for` iterate the two standard containers: `for x in v` over a `*Vector<T>` binds each element and `for k in m` and `for k, v in m` over a `*Map<K, V>` bind a key and a key with its value, all lowered before monomorphization into the index loop a program would write by hand against the containers' own accessors, and the `for` binder is now typed rather than a wildcard, recorded in the [Builtins](#builtins) chapter. 1.11.1 adds opt-in string interpolation: `f"total: {n}"` is an expression of type `string` whose holes render inline, byte-identical to what `println("{}", v)` writes for each value, checked against the same printable set a `println` argument takes, recorded in [Strings](#strings). 1.12.0 is a consolidation release that tightens a batch of rules a systematic sweep of the whole surface found silently loose, every one of them raise-only so the previous release still builds this source: inside a generic body a value of a bare type parameter borrows, so `free` and `move` on one are refused exactly as on a concrete borrow at every binding position, a builtin call name is reserved from a function declaration and from every other binding site, an interface value embedded at any depth inside a returned type is refused like a bare interface return, a brace written as a Unicode escape in a format string is a literal brace distinct from a placeholder brace typed directly, a lambda literal cannot be called where it is written, and `sizeof` of an inline tuple type is refused. The same release widens one rule: an `async func` may now take a `using` allocator, which lands in the task frame and backs `alloc` across every suspension. Each is recorded in [Memory Management](#memory-management), [Functions](#functions), [Strings](#strings), and the [Builtins](#builtins) chapter. 1.13.0 makes the four unsigned widths real surface after reserving their names since 0.5.4: `uint8`, `uint16`, `uint32`, and `uint64` are types an annotation names, `u8` through `u64` are literal suffixes that pin a literal to one, and the four names join the cast builtins. Signedness never mixes silently, the same rule width already carried, so a `uint32` beside an `int32` is a compile error naming which of the two rules broke, a negative literal never adapts into an unsigned type, unary minus on an unsigned value is refused, an unsigned index is refused, and division, remainder, right shift, the ordered comparisons, printing, and `hash` each take their unsigned form. Recorded in [Type System](#type-system), [Arrays and Slices](#arrays-and-slices), [Expressions and Operators](#expressions-and-operators), [Numeric Casts](#numeric-casts), and [Hash](#hash). 1.13.2 closes a batch of rules the implementation had left looser than the language means them: a `ref` marker on a destructuring binding borrows every name it binds, down the member paths of a nested tuple, a constant an expression of bare literals folds to is ranged against its target width exactly as a written literal is, a tuple literal member that names an owner hands the binder a borrow rather than the ownership, and an interface method signature naming a generic instance dispatches through the vtable instead of dying in `clang` while an interface declared with type parameters of its own is refused. An array literal whose element count is not the annotated `N` is refused rather than handed to `clang` as a differently sized aggregate, and a field access or an index whose base has no fields and no elements is refused with it. An enum's own name is reserved at every binding position, since a field read through such a binding lowers to that enum's constructor. The same release writes down the parser's nesting budget, which now charges each link of a folded chain and reports once. Recorded in [Memory Management](#memory-management), [Type System](#type-system), [Object Oriented Concepts](#object-oriented-concepts), [Arrays and Slices](#arrays-and-slices), and [Expressions and Operators](#expressions-and-operators). 1.13.3 adds four spellings the surface had gone without, none of them a new idea and each of them a shape a program reaches for constantly: `++` and `--` are expressions now, in a prefix and a postfix form and legal anywhere an expression is, so `arr[idx++]` steps the index in place and yields the old one; a `;` ends a statement exactly where a newline does, two statements fit on one line, and an empty statement is refused rather than dropped; an `if` used as a value types its branches and judges their ownership with the one voice a `match` expression already speaks, with an `else` required and each branch's value read off its block's tail; and `c ? a : b` spells that same selection as the loosest operator on the ladder, right associative and parsed into the same node, so the two spellings are one construct with one set of rules. Recorded in [Expressions and Operators](#expressions-and-operators) and [Statements and Semicolons](#statements-and-semicolons). The same release closes three judgments the checker had been letting through on a type it never resolved: a constructor call of a non generic enum carries that enum's type on the surface pass, so an annotation, a return, an assignment, an argument, a print, and a comparison against one are each judged where they are written rather than reaching `clang` or running on a value nobody meant, a generic type parameter spelled in a value position is that parameter rather than a like named variant of some enum elsewhere in the program, and an enum handed where an interface is expected is refused instead of reaching code generation with no vtable to dispatch through. Recorded in [Sum Types (Enums)](#sum-types-enums), [Functions](#functions), and [Object Oriented Concepts](#object-oriented-concepts). The same release changes one thing an existing program's output says, a `bool` printing as `true` or `false` where it printed `1` and `0`, and settles four smaller shapes around it: a `match` written as a statement takes an optional comma after an arm whose body is a braced block, a doc comment ends at its first `*/` rather than nesting the way a plain block comment does, `export impl` is refused since an impl declares no name to export, and a struct field read answers with the field's declared type. Recorded in [Strings](#strings), [Sum Types (Enums)](#sum-types-enums), [Comments](#comments), [Doc Comments](#doc-comments), [Exports](#exports), and [Object Oriented Concepts](#object-oriented-concepts).
...
- A doc comment does not nest. The `/**` form [Doc Comments](#doc-comments) describes ends at the first `*/`, since its body is prose rather than code, so a sentence inside one may spell `cmd/*.dusk` or `a /* b` without opening a level nothing closes. The plain `/* */` form nests exactly as described above.
...
A block comment that opens with `/**`, with the next byte after the second star neither `*` nor `/`, is a doc comment: `/**/` is an empty plain comment, and `/***/` or a `/****` banner stays plain. In every other way it is an ordinary block comment, stripped before parsing and producing no token, so a doc comment never changes what `check`, `build`, or `ir` does. It differs from the plain form in one rule: a doc comment does not nest and ends at the first `*/`, because what it holds is prose, and prose that mentions a path like `cmd/*.dusk` would otherwise open a level the writer never closes and swallow the rest of the file. An unterminated one is the lexical error a plain block comment gives, `unterminated block comment`, reported at the opening `/**` rather than wherever the reader noticed. What it adds is a binding: the compiler remembers the block and attaches it to the declaration it precedes.
...
`dusk doc <file>` renders the module as markdown: every function, struct, enum, interface, and impl in source order, each with its signature built from the declaration and its prose from the doc, an undocumented item listed with its signature alone. A file with a `foreign` block also renders a Foreign Functions section, each declaration's head with its bound prose. Prose is copied verbatim. The command reads the one file, without loading imports or type checking, so an imported name renders as written. `dusk doc --json <file>` emits the same model as JSON with a stable shape, the form a language server or a doc pipeline consumes; a foreign function carries its own kind and the byte span of its head, keys added to the model without disturbing the existing ones, so a consumer that does not know them ignores them. Both forms refuse to emit from a broken module: a dangling doc block, an unknown tag outside an example, an `@param` that names no parameter of its item or repeats one, an `@return` on a function returning `void`, and an `@param` or `@return` on anything without parameters are each reported with the doc's own source position, and the command exits nonzero with nothing on stdout. The checks live in `dusk doc` alone; `check` and `build` accept a program regardless of what its doc comments say.
...
`export` marks a declaration that carries a name, so an `impl` block cannot take it: `export impl Display for Point` is refused, `an impl takes no export; its methods follow the receiver type`. An impl declares no name of its own and its methods are reached through the receiver type, so exporting the struct is what makes them callable from another file and the word on the block would mean nothing. A `foreign` block is refused for the same reason, `a foreign block takes no export; its functions are reached by their own names`, since the declarations inside it name C symbols the linker resolves rather than dusk names a file hands out.
...
An export that crosses a package boundary carries one rule more. A dependency's exported names are scoped to the files that import them rather than reaching the whole program, so two packages may each export one name; [Exports Across Packages](#exports-across-packages) states it.
...
Added in 1.15.1. `dusk build --release <file>` compiles the emitted `.ll`, the C sources of the dusk runtime, and every `@csource` at `-O2`. Without the flag all three compile at `-O0`, which is what every build did before this release and what a plain `dusk build` and a plain `dusk run` still do. The flag reaches `clang` and reaches nothing else: the pipeline ahead of the link is untouched, so `dusk ir` prints the same bytes under the flag and without it, `dusk check` reaches the same verdict, and two builds of one program differ only in the machine code `clang` chose. Optimization is what the C compiler does with the IR, never what dusk emits, so no rule in this spec means anything different under it.

The flag composes with `--lib`. `dusk build --lib --release <file>` writes the same archive and header the plain library build writes, with the module object, the runtime objects, and every `@csource` object inside the archive built at `-O2`; the generated header is byte identical either way, since a prototype states the ABI and the ABI does not depend on an optimization level. `dawn build --release` passes the flag through to the compiler unchanged, so a package build takes it with the same spelling. `dusk ir --target=wasm32` is unaffected, since it emits IR and never invokes `clang`. Neither tool's `run` takes the flag: `dusk run --release <file>` is refused, `dusk: run takes no --release; build --release <file>, then run target/dusk-out/<stem>`, and every word after `dawn run` belongs to the program's own argv as it always did. An unrecognized flag on `build` is refused by name rather than read as a path, `dusk: unknown build flag '--relase'; build takes --release and --lib`.

The runtime objects a link consumes are cached rather than recompiled. The first link in a tree compiles the runtime's C sources and every link after it reuses the objects on disk. An object lands beside the build's other output, `target/dusk-out/runtime-<source>-<key>.o` for a compiler build and the same spelling under `target/dawn-out/` for a package build, one object per runtime source. The key is 16 hex digits over the first line of `clang --version`, the optimization level, and a hash of the runtime's own text, every runtime header folded into every key and each source's own bytes folded into its own, so an edited header misses on all of them, an edited source misses on exactly one, and a `clang` upgrade or a switch between a plain and a release build misses on all of them alike. A plain build and a release build in one tree keep their own objects side by side. An entry is written under a temporary name and renamed into place, so a reader sees a whole object or none, and two builds running in one tree cannot hand each other a partial file. A cache that cannot be read or written is never a failed build: every step falls back to handing `clang` the source directly, the way a link worked before the cache existed. What the key does not cover is anything outside the tree, a system header or an include path among them. The cache lives entirely under the build's output directory and has no expiry and no pruning command: removing `target/` removes it with everything else the build wrote, and that is the reset when the machine's own headers moved under a `clang` that did not.
...
- `@dusk <major.minor>` is the minimum compiler. The two numbers compare numerically against the running compiler's own and an older compiler refuses the build, `package.dawn asks for dusk 1.99, this is 1.14.0`. The floor is a major and a minor and nothing else, so a third number is refused rather than rounded off, `@dusk value '1.14.0' is not a version like 1.14`. A newer compiler is always accepted, since the language only widens. Since 1.14.1 a dependency's own floor is checked the same way, when its alias is first reached, and reported under its own manifest, `dawn_modules/<alias>/package.dawn: package.dawn asks for dusk 9.9, this is 1.14.1`, with nothing resolving through that dependency afterward; 1.14.0 read a dependency's floor and enforced only the project's, so a checkout written against a newer language failed somewhere in the middle of its source instead of at the line that said which language it was.
...
The graph is flat. dawn reads each dependency's own `package.dawn` and resolves that package's `@require` lines into the same `dawn_modules`, so a dependency of a dependency is a sibling of it rather than nested underneath. Four rules settle what happens when two manifests ask for the same thing, or for different things under one name, and each refusal names both manifests that asked. A source and reference pair already resolved is reused, so a package two dependents share is fetched and checked out once. One alias naming two different sources is refused, `alias 'p' is required from 'github.com/a/p' by package.dawn and from 'github.com/b/p' by dawn_modules/util/package.dawn; one alias names one package`. One alias naming one source at two references resolves to the root manifest's pin when the root declares that requirement, which is how a project overrides what a dependency asked for, and is refused otherwise, with the fix in the message, `alias 'p' is required at 'v1.0.0' by dawn_modules/a/package.dawn and at 'v2.0.0' by dawn_modules/b/package.dawn; add a @require for it in package.dawn to settle the version`. One source and reference reached under two different aliases is refused too, `'github.com/a/p' at 'v1.0.0' is required under two aliases, 'p' by package.dawn and 'q' by dawn_modules/util/package.dawn; one package has one alias`, since one checkout cannot answer to two names. The alias namespace is one namespace across the whole graph, unchanged through 1.14.1, so a dependency's choice of alias is visible to the project resolving it.
...
A dependency's own `@dusk` floor is checked when its alias is first reached, by the same reading the project's own floor gets, and a checkout asking for a compiler this is not stops there, `dawn_modules/greet/package.dawn: package.dawn asks for dusk 9.9, this is 1.14.1`, with nothing resolving through that dependency afterward. A package pins the language surface it was written against, so what sits in the checkout is source for a language this compiler does not speak and reading it as if it were this one's is the worse answer.
...
Since 1.14.1 an export belongs to the package that declares it. Every name a dependency exports is renamed to `<name>__<alias>`, with the alias the requiring manifest declared, throughout that dependency's own files, and every use inside the package follows the rename, so a package goes on calling its own functions by their own names while the merged module carries one distinct symbol per package. The rename reaches the emitted symbol: a dependency's exported `parse` is `@parse__maybe` in the IR, which is what lets two packages that both export `parse` land in one module and link. An alias is manifest text, so the symbol a build emits stays a function of the lock rather than of the machine.

What a bare name means is decided by the file that writes it, and what one file may spell bare is a scope of four layers. Its own declarations. Every other file of the project, since the project is one package and its files reach each other's exports with no import between them, exactly as they did in 1.14.0. The standard library the project reached, which is every std module any file of the project imported, since the project is one scope. And the exports of each dependency the file's own `@import` reached, the module form and the leaf symbol form alike, a leaf import bringing the one symbol it named and not the rest of that package's surface. A dependency's file has the same shape with the project's layer taken out and its own std layer in place of the project's: its own package's exports, sibling files included and no import needed between them, the std modules that package's own files imported, and the exports of the dependencies it imports itself. A project name is not visible inside a package.

Where two layers answer one name, a package's own wins. A name any file of the project declares beats a dependency's export of that name, statically and everywhere, so adding a `@require` cannot change what a name in the project already means, and the rule is symmetric: a package's own export, a sibling file's included, beats any standard library name, so a package that names a function after one of its own is not answered by std inside its own files. What is left is the conflict between two scopes neither of which owns the file: an export of an imported package against a standard library name, on either side of the boundary. A std module only a dependency imported is in the program without the project ever having reached for it, and a std module only the project imported is in it without the dependency having reached for it, so neither answers a name an imported package also claims. A file spelling such a name is told about both and picks, and nothing is ever chosen for it silently.

The qualified call folds through the same rename. `alias.name(x)` becomes `name__alias` when `name` is one of that package's renamed exports, and stays the bare name when it is not, which is what keeps a qualified call reaching a function whose spelling the rename deliberately left alone: `cnum.cabi_tag()` reaches a dependency's `export "C"` function under the name the linker knows it by. Resolution follows the order a program reads in, so a `do` step's initializer is resolved before the binder that step introduces and `value <- value()` calls a dependency's `value` and binds the new name after it.

Six refusals fall out of the scope. Each is raised at the reference that spelled the name, and each names its fix.

- `'parse' is exported by both 'a' and 'b'; qualify it`, for a bare name two of this file's imports both export. The file asked for both, so the compiler picks neither. One package exporting the name through `export "C"`, whose spelling the rename leaves alone, and the other through an export the rename moved is the same refusal: two imports answer the name and the file has to say which.
- `'maybe_tag' is exported by package 'maybe'; import it`, for a bare name a package this file's own package requires exports and this file did not import.
- `'leaf_tag' is exported by package 'leaf', which this package does not require; add a @require for it, then import it`, the same shape where the package is only in the build because something else pulled it in, so there is a manifest line to write before there is an import to add.
- `'trim' is exported by package 'su' and declared by 'lib/std/string.dusk'; import one or qualify it`, for the case above, an imported package's export meeting a standard library name. The package that claims the name is the one named, and the file that declares the other is spelled out beside it. It is raised in a project file, where the std module came in with a dependency, and in a dependency's own file, where it came in with the project, and it never fires for a name the file's own package exports, since that one is already settled.
- `'app_helper' is declared by the project, not by this package; a package cannot reach it`, for a dependency spelling a name only the project declares. The project's declarations are physically in the program the dependency compiles into, and a package written without them cannot depend on them.
- `'util' names two modules; import the one this file means`, for a qualified prefix two of the program's imports gave two different meanings, written in a file that imported neither.

Inside a dependency's own file the three shapes read this way. Its own package's export, moved by the rename or fixed as an `export "C"`, against a standard library name of that spelling is settled by the package, statically and with no sentence raised. The package's own scope is the innermost one its files read, so a bare use inside that package names the package's export and never the standard library's, whatever the standard library also spells. Its own package's export against an imported package's export of that name is `'foo' is exported by both 'p' and 'q'; qualify it`, since a sibling file's claim and an import's stand on equal footing where neither is the file's own declaration. And an imported package's export against a standard library name is the `import one or qualify it` sentence above.

The remedy each of those names is a spelling that always resolves. A qualified call reaches through a collision from either side, `ca.foo()` for a package's `export "C"` function and `std.vector.vec_len(v)` for the standard library's, so a file with two claims on one bare name says which it means and builds.

One package cannot export one name twice. A package exporting `foo` as an ordinary export and again through `export "C"` puts two claims on one name inside a single scope, which no import and no qualification can tell apart, so it is refused at the second declaration, `'foo' is exported twice by package 'p', once as export "C"; a package exports a name once`.

One more is earned by a manifest rather than by a reference. The loader gives a file's own private names a suffix of the form `<file stem>_<number>`, and an alias spelled like one of those would make one symbol out of two declarations, so it is refused at the line that declared it, `package.dawn:4: alias 'json_1' collides with an internal name; choose another alias`, rather than left to surface as a duplicate definition inside someone else's file.

Five kinds of name keep their spelling, each because the rename would break what the name stands for. A function declared `export "C"` and every declaration in a `foreign` block name a C symbol the linker resolves, so the name in the object file is the contract and not the compiler's to decorate; within a package a file reaching its sibling's `export "C"` function gets that function, and `ca.foo()` reaches it through the alias. A struct field, a method, and an enum variant are reached through a value whose type already says which package it came from. An exported `bind` and `unit`, declared as `T.bind` or bare, are the shape `do` notation rewrites into and resolve by that spelling, so the pair a package exports keeps it. An unexported pair is untouched by any of this and keeps the per file renaming it has had since before 1.14.0, so one file's private `bind` is not another file's to use, `undefined name 'bind'`, exactly as it was, and two files each keeping a private pair of their own stay legal. And a builtin, `print`, `println`, `alloc`, `free`, and `hash` among them, along with every primitive type name, is never renamed, so a package exporting one is refused by the reservation rule that already governs the name rather than scoped into coexisting with it. A file's own unexported function may carry a builtin's name as it always could, since nothing outside that file ever spells it. Two packages colliding on one of these is refused the way every cross package collision was in 1.14.0, the loader naming both files, `'json_tag' is also exported by 'src/main.dusk'; two packages cannot export one name`.

Three limits are named here rather than implied. A diagnostic about a dependency's export may spell the name with its suffix, `parse__maybe`, where the message is built from the merged module's own name rather than from what the file wrote. The alias qualified form is call shaped: `alias.name(x)` reaches a function, while `alias.Type` in a type position and `alias.Enum.Variant` in a pattern are not spellings the parser takes, so a dependency's type, enum, and variant are reached by their bare names through an `@import` of the module or the symbol. And the sentences above are raised at identifier positions, an expression, a variant path like `Color.Red`, and the head of a struct literal among them; a type annotation carries no span the load pass can raise at, so a dependency's exported type spelled bare in a file that did not import it reaches the type pass and is reported there as an unknown type rather than with the sentence naming the package that exports it.
...
Removed in 1.15.1. Before 1.14.0 a dependency was named by a quoted git path at the import itself, `@import "github.com/user/repo/module"`, resolved against a global clone cache with no reference and no lock behind it. 1.14.0 refused the form inside a project holding a `package.dawn` and left it resolving outside one for a release; 1.15.0 carried it one release further, since removing a way to spell an import is a behavior change and that release made none. It resolves nowhere now.

An import whose value carries a `/` is refused wherever it is written, inside a project and outside one alike, `'example.com/user/repo/mod' is a url import; declare it with @require in package.dawn`. That is the sentence 1.14.0 already printed inside a project, and it is now the answer everywhere, since there is one way a dependency reaches the compiler and it is a `@require` in a manifest with a commit in the lock beside it. The loader no longer reads the `DAWN_CACHE` root at all, so no import resolves against a directory outside the project that no lock stands behind. dawn keeps its own use of that variable for the bare mirror cache a fetch clones out of, which is a fetch time optimization and never an import path; the two shared a name and were never the same mechanism.

A program on the old form moves in two steps, both of them ordinary: write a `@require` for the repository at the reference you want, and spell the import through the alias the require declared.
...
Added in 1.13.0, after the four names spent the bootstrap reserved. `uint8`, `uint16`, `uint32`, and `uint64` hold a nonnegative value of their width, and the suffixes `u8`, `u16`, `u32`, and `u64` pin a literal to one without an annotation, the way `i8` through `i64` already do for the signed widths. Arithmetic wraps at the width exactly as signed arithmetic already wraps, so an unsigned type adds no new trap and no new fault.

An unsigned type is a distinct type, not a signed one wearing a flag. `uint32` and `int32` both ride an LLVM `i32` and still never mix, never compare, and never assign across each other, and a generic instantiated at an unsigned width is its own instantiation with its own emitted body, distinct from the signed one of the same width. Everything sign sensitive follows the type: division and remainder take their unsigned form, `>>` shifts a zero into the top bit rather than copying the sign bit down, the four ordered comparisons read the top bit as magnitude rather than as a sign, widening zero extends, an unsigned value crossing to a float goes by its magnitude, printing writes unsigned decimal, and `hash` widens by zero extension. Addition, subtraction, multiplication, the bitwise trio, `<<`, and equality are the same instruction under either signedness and answer the same for the same bits.

```text
count: uint32 = 4000000000            // no signed 32 bit width holds this
tag := 7u8                            // a uint8, pinned by the suffix
top := 18446744073709551615u64        // the top of uint64, written directly
mask: uint16 = 0u16 - 1u16            // wraps to 65535, the same wrap a signed width takes
```

An unsuffixed integer literal is read as an `int64`, so a bare decimal above `9223372036854775807` is `invalid integer literal`. A `u` suffixed literal names a bit pattern rather than a signed magnitude and ranges against the unsigned top instead, which is what lets `18446744073709551615u64` be written at all; above `2^64 - 1` the reject stands whatever the suffix says. A suffixed literal too wide for the type it lands in meets the ordinary fit rule and names the value it read, so `w: uint32 = 18446744073709551615u32` is `literal 18446744073709551615 does not fit in uint32`.

Unary minus takes a signed integer or a float. Negating an unsigned value would wrap to the top of its range rather than going below zero, which is almost always a width bug, so it is rejected at check, `cannot negate uint32; an unsigned value has no negation, cast to a signed width first`. Write the cast when the wrap really is the intent.

An unsigned value crosses the foreign boundary as the same LLVM `i<w>` the signed width of that size crosses as, so a `foreign` declaration may name one as a parameter or a return type. The generated C header a `dusk build --lib` writes spells it `uint<w>_t`, since the header is what a C caller compiles against and `int<w>_t` there would make the C side read a large unsigned value as negative.
...
Numeric widths never mix silently, and neither does signedness. Arithmetic, comparison, assignment, and argument passing take operands of one width and one signedness, so an `int32` next to an `int64` is a compile error rather than a truncation, and an `int32` next to a `uint32` is a compile error rather than a reinterpretation. The diagnostic names which of the two rules broke: a width disagreement ends `; match the widths`, as in `arithmetic mixes int32 and int64; match the widths`, and a signedness disagreement at one width ends `; cast to one signedness`, as in `arithmetic mixes int32 and uint32; cast to one signedness`. Both tails carry across every operator family that takes two integers, so a comparison, a bitwise operator, a shift, and `**` each report the same way, `comparison mixes uint32 and int32; cast to one signedness`, `'&' mixes uint32 and int32; cast to one signedness`, `'**' mixes uint32 and int32; cast to one signedness`. The conversion between two signednesses is an explicit cast and nothing else.

A bare literal adapts to the width and the signedness beside it, so `5u32 + 1` is a `uint32` and the unsuffixed operand never drags the result back to a signed twin of the same width. A literal that cannot fit its annotated type is rejected, a signed width taking the range `[-2^(w-1), 2^(w-1)-1]` and an unsigned width taking `[0, 2^w - 1]`: `x: int8 = 300` is `literal 300 does not fit in 8 bits`, and `x: uint8 = 256` is `literal 256 does not fit in uint8`, the unsigned form naming the type rather than a bit count since the bit count alone would read as though the bit pattern were the question. A negative literal has no unsigned value at any width and is refused outright, `counter: uint32 = -1` giving `literal -1 does not fit in uint32; an unsigned type holds no negative value, cast if the wrap is meant`. The rule follows the type into every position a literal adapts into: an annotated binding, an assignment or a compound assignment, a call argument, a return, a struct literal field, an enum constructor payload, an array or slice literal element at any nesting depth, a tuple literal member against an annotation or a return, a destructured binder, the tail of a `match` arm, a bare operand beside an unsigned value, so `-1 + u` is refused where `u` is a `uint32`, and a `collector<T>` mint.

Since 1.13.2 the same range judges a constant an expression of bare literals folds to. Codegen computes such an expression at `int64` and truncates the result into the target, so the folded value is exactly the value the target width has to hold, and ranging it asks precisely whether that store would truncate. `x: uint8 = 128 + 128` is `constant 256 does not fit in uint8` exactly as the written `256` is, `y: int8 = 100 + 100` is `constant 200 does not fit in 8 bits`, `m: uint64 = 0 - 1` is `constant -1 does not fit in uint64; an unsigned type holds no negative value, cast if the wrap is meant`, and the rule follows the fold into every position above, so a call argument written `take(2 * 200)` against a `uint8` parameter is `constant 400 does not fit in uint8`. The noun is the only difference from the written form's message, `constant` for a value computed rather than typed, and the caret covers the whole expression, since no single literal in it is the one that did not fit. The fold reads `+`, `-`, `*`, `/`, `%`, and unary minus over bare literals and nothing else. A suffixed operand stops it, so `0u64 - 1u64` computes in its own width, where wrapping to the top of the range is the deliberate spelling of that value, and stays legal; a cast stops it the same way, so `uint8(0) - uint8(1)` is the explicit wrap; a name and a call stop it because neither carries a value the pass knows. A fold that leaves `int64`'s own range is judged apart from the ordinary fit: a `uint64` holds every `int64` bit pattern and the store keeps the bits the fold computed, so `a: uint64 = 9223372036854775807 + 1` is `9223372036854775808` and is accepted, while a target that cannot hold the result is refused, `constant expression overflows int64 and does not fit in uint8; cast if the wrap is meant`, naming the target and the cast rather than a value, since what the fold holds after an overflow is a wrapped pattern and not the number anyone wrote. A fold that never overflowed and simply names a negative is refused at `uint64` as it is at every other unsigned width, so `0 - 1` stays `constant -1 does not fit in uint64; an unsigned type holds no negative value, cast if the wrap is meant`. A shift, the bitwise operators, and `**` are never folded, since a mask written as a shift is the ordinary way to name a value at the top of a width. The two shapes with no `int64` answer, a zero divisor and the one signed overflow of `int64`'s low value over `-1`, are left unfolded and stay with the runtime, which is where codegen already leaves them.

Literal suffixes select a non default type without an annotation. The `i` spellings name a signed width, the `u` spellings an unsigned one, and `f32` or `f64` a float.

```text
a := 3.14f32    // float32
b := 5i8        // int8
c := 200i16     // int16
d := 200u8      // uint8, a value no int8 holds
e := 5u64       // uint64
```
...
A `char`, a `char[N]`, and a `char[]` print as the text they are, not as numbers. `print`, `println`, and `printerr` write a `char`'s single byte, a `char[N]`'s `N` bytes, or a `char[]`'s bytes straight to the stream, exactly as given, so `s := "hi"; println(s[0])` prints `h` and a `char[5]` holding `['H','e','l','l','o']` prints `Hello`. A format hole, `{}`, prints the same way when the value behind it is one of these three types. The bytes are written whole, not scanned for a NUL and not decoded, so an embedded NUL prints through and a multibyte UTF-8 sequence prints its glyph intact rather than one raw byte's worth of garbled text. Reading the numeric value behind a `char` is one annotated binding away, `b: int64 = c`, unchanged from before. `rune` does not follow this rule; see [Runes and Unicode](#runes-and-unicode) for why `println(rune)` still prints a codepoint number. The `{}` format hole and the interpolation hole [Interpolated Strings](#interpolated-strings) describes share one printable set and one rendering, so a value prints the same bytes whichever writes it.

A `bool` prints as `true` or `false`. `print`, `println`, and `printerr` write the word, and a `{}` format hole and an interpolation hole write the same bytes, so `println(x > 0)` prints `true` where it printed `1` before 1.13.3. The words are what the value means, and a program that reads its own printed output reads them now.

A brace a `println` format string produces through a Unicode escape is a literal brace, distinct from a placeholder brace typed directly. `\u{7b}` and `\u{7d}` name the codepoints for `{` and `}`, so each decodes to one literal brace the format scan writes straight through rather than reading as a placeholder or a doubled brace: `println("A\u{7b}B")` prints `A{B`, `println("\u{7b}\u{7b}")` prints two braces with no fold to one, and a real placeholder still expands between escaped braces, so `println("val=\u{7b}{}\u{7d}", 5)` prints `val={5}`. A brace typed directly still carries its format meaning, `{}` a placeholder and `{{` a doubled literal brace, so only the escape spelling names a literal brace unambiguously. This matches the interpolated string rule, where a chunk's escapes decode before its braces are read, and it distinguishes a brace that is text from a brace that is syntax rather than folding or misreading the two together.

### Interpolated Strings

Added in 1.11.1, an interpolated string is a `string` valued expression written with an `f` prefix on the opening quote, `f"total: {n}"`, following the `r'...'` rune prefix precedent. It renders each hole inline and produces an ordinary heap `string`, the same allocation class `+` and `substring` return, so the result frees with an ordinary `free` and feeds the string `+` operator directly.

A chunk is the literal text between holes and reads exactly like a plain string literal: the same escapes decode, `\n`, `\t`, and `\u{...}` among them, and a doubled brace is a literal brace, `{{` cooking to `{` and `}}` to `}`. A hole is `{` an expression `}`, and that expression is an ordinary expression of any shape, a call, a field chain, an index, arithmetic, a struct literal, or a nested plain string literal whose own braces the lexer balances so the hole closes at the matching brace rather than the first one. Holes may sit adjacent, `f"{a}{b}"`, and a chunk may be empty, so a leading, trailing, or between-holes empty chunk renders nothing. A holeless `f"abc"` is still an interpolated string, a single cooked chunk with no hole; it produces a fresh heap `string` like any f-string and stays a value rather than a format string, so its doubled braces cook the same way and `println(f"{{}}")` prints a literal `{}`.

```text
n: int64 = 5
s: string = "hi"
msg := f"n={n} s={s} and a literal {{brace}}"   // "n=5 s=hi and a literal {brace}"
free(msg)
```

A hole's value is written with the same printable set a `println` argument takes: an integer of any width, signed or unsigned, a float, a `bool`, a `char`, a `char[N]`, a `char[]`, a `rune`, a `string`, an `error`, and a struct with a `Display` `toString`. The rendered bytes are byte-identical to what `println("{}", v)` writes for the same value, minus the newline, for every one of these types, a struct rendered through the same `toString` path. A value with no printable form is rejected at the hole with the same diagnostic the format hole gives: an enum is `cannot print the enum '<name>'; match on it and print its parts`, a managed or raw pointer is `cannot print a pointer; dereference it or print its fields`, and a `void` value is `cannot print a void value`, so `f"c={c}"` over an enum, `f"p={p}"` over a pointer, and `f"x={nothing()}"` over a void call each fail at check.

An interpolated string is not a format string. It carries no `{}` placeholder and no trailing argument list; every brace pair either doubles to a literal brace or holds an expression. As a `println` argument, `println(f"x{n}")`, it is one already-rendered string value passed down the ordinary value path, not a format string the call expands, so `println` never scans it for holes.

An interpolated string is rejected at five points, each with a named diagnostic. An empty hole has no expression to write and is refused at parse, `an interpolation hole needs an expression; '{}' is the format placeholder for println, an f-string writes the expression inline`, which distinguishes it from the `{}` a format string uses. A second interpolated string inside a hole is refused at lex, `an interpolated string cannot nest inside another; bind the inner one first`, so a nested `f"..."` binds to a name first. A bare closing brace in a chunk is refused at lex, `unmatched '}' in an interpolated string; write '}}' for a literal brace`, stricter than the format language, which passes a lone `}` through, a deliberate asymmetry that keeps a stray brace from reading as text. An interpolated string with no closing quote is `unterminated string literal`, the same reject a plain string gives. And an `await` inside a hole is refused where every mid-expression `await` is, `'await' cannot appear mid-expression; give the awaited value a name, as in v, e := await f`, since a hole is an ordinary expression and `await` is legal only in statement position; bind the awaited value first.
...
- An array literal fills a fixed array exactly. A literal builds at its own length, so a count that is not `N` stores a differently sized aggregate into the slot and is rejected at check: `r: int64[8] = [0]` is `the array literal has 1 element(s) but int64[8] holds 8; a short literal is not padded, write every element`, and `r: int64[2] = [1, 2, 3]` is `the array literal has 3 element(s) but int64[2] holds 2; drop the extra elements or widen the type`. A short literal is not padded and there is no fill form, nor an initializer less declaration to reach for instead, so every element of a fixed array is written. The count rides the same target directed literal walk the numeric fit rule rides, so it lands at every position that walk reaches, an annotated binding, a destructured binder, an assignment, a return, a call argument, a struct literal field, and a `collector<T>` mint, at any nesting depth inside another literal, and the elements are still checked against the element type after a wrong count reports once. A slice target counts nothing, since a slice takes the literal's own length, and a `char[N]` filled by a string literal keeps its own byte count rule; see [Strings](#strings).
...
- An index and a range endpoint are `int64` offsets. An unsigned one is rejected at check, `cannot index with uint32; an index is int64, cast the index`, at both `xs[at]` and `xs[from..2]`, rather than left for the widening to pick an extension by a signedness the index position never states. The cast is the one word fix, `xs[int64(at)]`.
...
A binding is visible from its declaration to the end of the block that introduces it. A nested block opens its own scope: the body of an `if` branch, a `while` or `for` body, and a `match` arm each hold their own bindings, and a binding there shadows an outer binding of the same name for the rest of that block. When the block ends the outer name is visible again with its own value and type. Name resolution walks from the innermost open block outward to the function frame, so the nearest binding of a name wins.

```text
x: int64 = 1
if true {
    x: int64 = 2      // shadows the outer x inside this branch
    println(x)        // 2
}
println(x)            // 1, the outer binding is visible again
```

Shadowing is unrestricted across a block boundary. The inner binding may take a different type than the one it shadows, an inner `string x` over an outer `int64 x`, and the outer binding keeps its type after the block, so `x + 1` after the branch reads the outer `int64`. A `for` loop variable, a `match` arm's payload binder, and an ordinary binding inside a branch are block scoped the same way, so reading the outer name after the construct reads the outer value, not the last one the block bound.

A lambda captures the bindings visible at the lambda's position. A name a lambda reads resolves against the scope open where the lambda is written, so a lambda placed after a shadowing block captures the outer binding the block restored, not the block's binding that has already ended.

Name resolution is one model across the whole compiler. The type checker and code generator resolve every identifier against the same scope chain, in every construct, at every capture site, and at a `defer` replay, so a program that checks clean lowers against the identical bindings. A `defer` is the one place resolution deliberately does not follow the innermost open block, and its rule is stated with [`defer`](#scope-cleanup-with-defer) below.

### Statements and Semicolons

A statement ends at a newline, the rule dusk has carried since the beginning, and since 1.13.3 a `;` ends one too, exactly and only where a newline would. `a = 1; b = 2` is two statements written on one line, and a `;` at the end of a line is a terminator the newline was going to supply anyway, so writing it changes nothing about what the line means.

```text
a: int64 = 1; b: int64 = 2      // two statements, one line
c: int64 = 3;                   // a terminator the newline already gave
return x;                       // a return takes the terminator like any statement
```

A `return` ends with a `;` the way every other statement does, and a bare `return;` is legal beside `return x;`: the `;` terminates the statement and never stands in for the value a bare `return` deliberately does not carry.

The `;` is strict about having a statement to end. An empty statement is refused wherever a statement begins, `;;`, a line holding a lone `;`, and a `;` written immediately after a `{` among them, `stray ';'; a semicolon ends a statement, and there is no statement before it`, and the same message answers a stray `;` at the head of a `do` block's step, the other position a statement starts at. A `;` after a top level declaration answers to its own message, since the terminator has no meaning there at all rather than merely nothing in front of it: `stray ';'; a top level declaration does not end in a semicolon`. The stray token is consumed either way, so each one is named once and the loop reading the file still makes progress, and a run of them reports a diagnostic apiece instead of derailing into an unrelated fault. There is no other way to write an empty statement in dusk, so the language has one answer for the shape rather than a token it silently drops.

There are no insertion rules. dusk never supplies a `;` a program did not write, and a `;` never changes how a line continues, so the newline rule stands exactly as it stood: it is what ends a statement when no `;` is present, and a construct that spans lines spans them for the reasons it always did. Nothing here resembles the automatic semicolon insertion JavaScript performs, where the language guesses at a terminator the source omitted and a program's meaning turns on the guess.

Two declaration forms keep an optional `;` they have always admitted, an interface method signature and a `foreign func` declaration, each of which may carry one after its return type. Both are declarations rather than statements, so the terminator rule leaves them exactly as they were and the empty statement refusal does not reach them.
...
- `<stem>.h`, a generated C header: `#include <stdint.h>`, one prototype per export inside an `extern "C"` guard so a C++ host may include it too, and a comment naming the link line. Each prototype's C types come from the same lowering the emitted symbol does, `int64_t` for an `int64`, `uint32_t` for a `uint32`, `uint8_t` for a `char`, `int32_t` for a `rune`, `double` for a `float64`, `void*` for a `*raw T` or a `*void`, so the header can never drift from the archive. The unsigned spelling matters even though the LLVM type is the same `i<w>` under either signedness: the header is what a C caller compiles against, and an `int<w>_t` there would make the C side read a large unsigned value as negative.
...
- `match` is exhaustive. A missing variant is a compile error, `non exhaustive match, missing variant 'None'`. The rule holds over a scrutinee whose type the surface pass cannot name as well: a constructor call, a binding a constructor initialized, and a place read all infer their type later, so the enum is recovered from the scrutinee expression itself, through the enum a binding was initialized from, through a qualified constructor written where it is matched, and through a field, index, or dereference chain whose declared type names one. The enum the scrutinee resolves to judges the arms themselves as well as their coverage: an arm naming a variant that enum does not declare is `'Opt' has no variant 'Circle'`, and a pattern binding the wrong number of payload names is `variant pattern 'Some' binds 2 of 1 payload field(s)`, each read off the resolved enum's own variant slot rather than a table of every variant name in the program. A payload carrying variant named alone is a constructor rather than a value of its enum, so `match Opt.Some { ... }` is `'Opt.Some' takes 1 argument(s), so naming it alone is not a value; call it with its payload`. A scrutinee that resolves to no enum at all is left unjudged, since there is no variant set to compare the arms against.
...
The comma after an arm is optional whenever the arm ends in a closing brace, in a `match` written as a statement and in one used as a value alike. The brace ends the arm the way the comma would, so `Circle(r) => { ... }` may run straight into the arm below it, and the same holds where the brace is the tail of a `match`, an `if`, a lambda, or a `do` the arm evaluates rather than the arm's own block. An arm that ends any other way still takes the comma, since nothing else marks where a one line arm stops, and a miss names the rule, `expected ',' after a match arm; an arm that ends in a block may omit it`.

A variant is constructed through the enum qualified form, `Shape.Circle(2.0)`. A `match` pattern reads the other way: an arm names the variant bare, `Circle(r)`, not qualified, since the scrutinee's own type already fixes which enum the arm belongs to. So construction is qualified and a pattern is bare, and the two never share a spelling. The bare form in construction position, `Circle(2.0)`, is not a constructor and is rejected, `use the qualified form 'Shape.Circle' to construct an enum value; the unqualified variant name is not a constructor`, naming the fix rather than resolving the variant by its global name and risking a collision with a like named function or a stale local of the same name still in scope. A constructor's argument count and each payload's type are checked against the variant's declaration: `Shape.Circle()` with no argument, and `Shape.Rect(1.0, true)` against a `float64` second field, are each rejected at the constructor site rather than left to surface later as an unrelated mismatch once the value is read back. A literal payload must fit the field's declared width the same way an annotation's right hand side must: a constructor handing an `int8` field a literal too large for eight bits is rejected at the constructor, `literal <n> does not fit in 8 bits`, the identical bounds rule `x: int8 = 300` faces. A constructor or a pattern that names a variant the enum does not declare is rejected the same way, `'<Enum>' has no variant '<name>'`, at both the qualified constructor `Shape.Oval(1.0)` and the bare pattern `Oval(r)`, rather than passing check and dying later in codegen on a variant that never resolves. Since 1.13.3 a constructor call of a non generic enum carries that enum's own type on the surface pass, where it inferred unknown before, so every question asked of the value it makes is answered where the constructor is written: `x: int64 = Opt.Some(1)` is `'x' has a type annotation that does not match its value`, a `return Opt.Some(1)` from a func returning `int64` is `return type does not match the function's return type`, an `o = Tag.Hi(1)` over a `mut o := Opt.Some(3)` is `assignment type mismatch`, a constructor of the wrong enum at a call is `argument 1 has the wrong type`, `println(Opt.Some(7))` is `cannot print the enum 'Opt'; match on it and print its parts`, and `Opt.Some(1) == Opt.Some(1)` is `cannot compare 'Opt'; compare its parts instead`. A payload free constructor answers with the call form beside it, so `Opt.None` and `Opt.Some(1)` agree about what they are. A generic enum's constructor stays a hole on the surface, since the named type erases its parameters there, and its instance is typed on the ground pass monomorphization drives.

An enum's own name is reserved at every binding position. A `let` or annotated binding, a destructure binder, a parameter, a lambda parameter, either binder of a `for`, a `match` arm's payload binder or catch all binder, and an `await` binder each refuse it, `'Opt' is an enum name; a binding cannot take it, because a field read through it names that enum's constructor, so rename this binding`. The reason is the constructor spelling: `A.Some` is read as a constructor off the base identifier's own spelling, with no scope consulted, so a binding named `Opt` made `Opt.Some` inside its scope lower to a zeroed constructor of the enum rather than the field read it looked like, and the program printed a value nobody wrote. The same shape through a parameter passed the checker with a payload of the wrong type, since the body read the declared enum's constructor whatever the caller handed in. Reserving the name leaves one answer for both. Only the enum's own name is taken, and the reservation is deliberately narrow: a variant name binds freely, since no lowering keys on a variant alone, a struct field may be spelled like a variant and `s.Some` stays the field read it looks like, and a payload free arm naming a variant of the scrutinee's own enum names that variant rather than binding anything, so it stays legal even where another enum in the program carries the same spelling. The variant reading and the binder reading are told apart by the scrutinee's enum, so where that enum cannot be named on the surface, inside a generic body over a type parameter, over the result of a generic call, or over a binding of either, the question waits for the ground pass monomorphization drives, where every type is concrete: an `A => ...` arm inside a generic body stays the variant it names even when the program also declares an `enum A`, and only a genuine catch all binder spelled with an enum's name is refused.
...
Added in 0.4.2. Every operator sits on one precedence ladder, fourteen levels from loosest to tightest, each level left associative unless noted. Parentheses group as usual, and only the comparison level rejects chaining outright: `1 < 2 < 3` is a compile error, not a silently wrong bool.

| Level (loosest to tightest) | Operators                                 | Notes                                     |
| --------------------------- | ----------------------------------------- | ----------------------------------------- |
| 1. Conditional              | `? :`                                     | right associative, loosest, see below     |
| 2. Range                    | `..` `..=`                                | only legal inside a slice index           |
| 3. Pipe                     | `\|>`                                     | a parse time rewrite to a call, see below |
| 4. Or                       | `\|\|`                                    |                                           |
| 5. And                      | `&&`                                      |                                           |
| 6. Comparison               | `== != < <= > >=`                         | not chainable                             |
| 7. Bitwise or               | `\|`                                      |                                           |
| 8. Bitwise xor              | `^`                                       |                                           |
| 9. Bitwise and              | `&`                                       |                                           |
| 10. Shift                   | `<< >>`                                   |                                           |
...
| 14. Unary, then postfix     | prefix `- ! ~ * ++ --`, then call, index, field, postfix `++ --` | tightest, unary binds tighter than `**`   |
...
Unary minus takes a signed integer or a float. An unsigned operand is rejected at check, `cannot negate uint32; an unsigned value has no negation, cast to a signed width first`, since the negation would wrap to the top of the range rather than go below zero; see [Unsigned widths](#unsigned-widths). Every binary operator that takes two integers requires them to agree on width and on signedness, and the diagnostic names which rule broke; see [Type Inference](#type-inference) for the two tails and the literal adaptation that goes with them.

The two postfix operators name a legal base set, and a base outside it is refused at check rather than lowered. A field access takes a struct, a struct read through a dereference written `(*p).f`, `self`, an enum's qualified variant such as `Color.Red`, a method call, a value whose type is still a bare type parameter, an array's or a slice's `.len`, and an error's `.message`; every other base is rejected, `cannot access a field of int64; a field access takes a struct, an array or a slice '.len', or an error '.message'`, with a string answered on its own terms since its byte count is a call rather than a field, `cannot access a field of string; a string has no fields, its byte count is str_len(s)`. An index or a range slice takes an array, a slice, a string, or a pointer to one of those, and any other base is rejected the same way, `cannot index int64; an index or a range slice takes an array, a slice, a string, or a pointer`. Both refusals cover a read and an assignment target alike, since the left side of an assignment is inferred through the same rule, and a value whose type is still a hole inside a generic body is left to the ground pass that runs once monomorphization makes it concrete.
...
`== != < <= > >=` compare two operands of the same type. An integer, a float, `bool`, `char`, `rune`, and a string all support the family, though a string supports only `==` and `!=`; a `<`, `<=`, `>`, or `>=` between two strings rejects, `strings compare with == and !=; they have no ordering`, since a string has no meaningful order to compare by. A string compares by content, through a runtime byte compare, not by the pointer identity every other value compares by; see [Strings](#strings). An integer comparison follows the operands' signedness: `<`, `<=`, `>`, and `>=` on two unsigned operands read the top bit as magnitude rather than as a sign, so the two halves of a width order the way the values do, while `==` and `!=` are the same instruction under either signedness.
...
`<<` and `>>` shift by an integer amount. `<<` is a plain logical shift under either signedness. `>>` follows the operand's type: on a signed integer it is an arithmetic shift, sign extending the top bit, and on an unsigned integer it is a logical shift that brings a zero down, so `2147483648u32 >> 1u32` is `1073741824` where the same bits read as an `int32` shift to `-1073741824`. A constant shift amount outside `[0, width)` is a compile error, a negative constant included. A dynamic amount is checked at the shift itself and a miss aborts with the named fault `fatal: shift amount out of range`, at the file and line of the shift itself since 1.2.0, never a silently masked or poison result. See [Runtime Faults](#runtime-faults).
...
Since 1.13.3 `++` and `--` are expressions, in a prefix and in a postfix form, and legal anywhere an expression is: `arr[idx++]`, `arr[++idx]`, `f(i++)`, `return ++n`, an array element, a struct literal field, and a bare `x++` on a line of its own. Postfix yields the value the place held before the step and prefix yields the value it holds after, so `arr[idx++]` indexes at the old index and then bumps the binding while `arr[++idx]` bumps first and indexes at the new one. Before 1.13.3 both were statement only and postfix only, so every program written against the older rule keeps its meaning exactly.

The operand is a place, a name, a field chain, an index, or a dereference chain, the same storage the left side of an assignment must name, since the operator writes the stepped value back. Anything else is refused naming the operator that asked, `'++' needs a place to write back to: a name, a field, an index, or a dereference`, so `(a + b)++` has nothing to step. The place's binding must be `mut`, the rule compound assignment carries, judged by the check the assignment forms run, so an immutable target earns the message it always has, `cannot assign to immutable 'x'` on the binding itself and `cannot assign through immutable 'x'; declare it 'mut'` through a dereference or a field of one.

The operand's type is an integer of any width, signed or unsigned. The step wraps at that width exactly the way `+ 1` and `- 1` already wrap, so an `int8` at 127 steps to -128, a `uint8` at 255 steps to 0, and `--` on an unsigned zero wraps to the top of the width rather than going below it, where a subtraction would have to go. Every other type is refused with the operator, the type, and the verb in one line, `'++' takes an integer place; float64 cannot be incremented`, and `--` says `decremented` in the same slot. A float has no unit step its width agrees on, a `char` and a `rune` are text, a `bool` carries no arithmetic, and a pointer is refused as `'++' takes an integer place; a managed pointer cannot be incremented`, since dusk has no pointer arithmetic and a generation check has nothing to say about an address a step invented.

The place is evaluated exactly once, the rule compound assignment already states: `xs[pick()]++` calls `pick()` once, and the index it returns is the one the read and the store share. Evaluation inside a single expression runs left to right, so an expression that steps the same place twice has an answer written down here rather than an undefined one: with `i` at 1, `i++ + i++` is `1 + 2`, so the expression is `3` and `i` is `3` after it. dusk leaves no evaluation order undefined, so the rule is a rule and not a warning against writing the shape.

The statement forms keep their meaning. `x++` and `x--` on a line of their own step the place and discard the value, and the unused value is not an error there, since a statement position is where a step is most often written for its effect alone. The statement form still folds to a compound assignment at parse time, and the fold records which operator wrote it, so the judgment is the operator's own in both positions: `f++` on a `float64` binding is `'++' takes an integer place; float64 cannot be incremented` written as a statement exactly as `arr[f++]` is written inside an expression, the same place check, the same integer rule, and the same messages either way.

### Exponent

`**` is right associative. An integer base and exponent lower to `cool_pow_i64`, repeated squaring in `uint64_t` so the wraparound matches the plain `mul` codegen already emits; `0 ** 0` is `1`, the same convention Rust's `pow` uses. A negative integer exponent is meaningless for an integer result. A constant negative exponent is rejected at check, `'**' on integers needs a nonnegative exponent`, before any code is emitted; a dynamic exponent that turns out negative at runtime keeps the named fault `fatal: negative exponent in integer '**'` rather than returning a wrong value. An unsigned base and exponent are the same repeated squaring and wrap at their own width the way the other unsigned arithmetic does, so `2u64 ** 3u64` is `8` and `65536u32 ** 2u32` is `0`, the exact power `4294967296` wrapped away at 32 bits. An unsigned exponent is never negative, so the runtime fault cannot arise on an unsigned pair however large the exponent runs: `1u64 ** e` for an `e` at the top of `uint64` is `1`, ordinary work rather than a trap. A float base or exponent lowers to the LLVM `pow` intrinsic at the operand's width.
...
`a..=b` in a slice index is `a..b+1`: the endpoint moves before the ordinary `lo <= hi <= base.len` bounds check runs, so `xs[2..=1]` is the empty slice rather than a trap, and `xs[0..=n-1]` covers the whole backing. The form belongs to the slice index and nowhere else: a counted `for` head takes the half open range only, described in [The counted loop](#the-counted-loop).

### The if expression

Added in 1.13.3. `if c { a } else { b }` is an expression as well as a statement, so a selection sits wherever a value goes: `x: int64 = if n > 0 { n } else { 0 - n }`, a call argument, an array element, a struct literal field, a `return`.

Each branch's value is its block's tail, the rule a `match` arm already follows. The last statement of a branch block, when it is a value, is what that branch yields, and a tail that is itself a bare `match` or a bare `if` value-izes into the selection around it, so a nested selection is the branch's value with no name bound to carry it. This nests to any depth, and the type agreement and the ownership verdict below penetrate every level. The rule runs both directions: a `match` arm's body that ends in an `if` carrying an `else` value-izes into the match exactly as a tail `match` does, its branches supplying what the arm hands up. An `if` with no `else` is a statement wherever it is written, there included, so an arm ending in one yields no value and answers to the match expression's own rule that every arm yields or diverges.

An `else` is required in the expression form, `an if used as a value needs an else branch`. A one armed `if` has nothing to yield when its condition is false, and a result slot no branch wrote is exactly the hole the match expression closed at 1.10.1. An `else if` chain is legal and carries no new rule, since a chain is an `else` branch whose whole body is one nested `if`; the chain's final `else` is the one that closes it, at whatever depth it sits.

Every branch yields a value or diverges. A branch whose block ends in a statement that stores nothing, a one armed `if` or a `while` among them, and that does not `return`, `break`, or `continue` out of the selection is rejected, `an if used as a value needs every branch to yield a value or diverge`, the same rule and the same reason a match expression's arms answer to. A branch cannot yield a lambda literal either, `a branch of an if expression cannot yield a lambda; bind the function to a name outside the if`, since the result slot has no shape for a closure value, the refusal a match arm already carries.

The branches agree on type under the match arm rules. Width and signedness agree, a bare literal adapts to the branch beside it the way it adapts to any other operand, and a branch whose value disagrees in width, in signedness, or in kind, a pointer beside an integer for one, is rejected, `the branches of an if expression must agree on type`. A branch that is a bare literal is ranged against the type the branches settled on, the rule any literal faces at a binding: with `a: int8 = 5` in scope, `y := if c { a } else { 300 }` is refused, `literal 300 does not fit in 8 bits`, rather than truncated into a result slot the other branch sized. The unsigned form of the message names the type instead of a bit count, `literal 300 does not fit in uint8`, the same pair [Type Inference](#type-inference) already describes. That agreed type is the expression's own type, so an `if` bound or passed against an annotation is width and type checked like any value, and an `if` used as another selection's branch value carries its type into the outer agreement.

Ownership is judged across the branches with one voice, exactly as a `match` expression judges its arms. The result is an owner only when every branch yields an owner, an `alloc`, a `move` of a binder, a branch local fresh owner, or an owning call among them, and a borrow only when every branch yields a borrow; branches that disagree are rejected at the odd branch, `the branches of an if expression must agree on ownership of its result`. A borrowed result is refused a `free`, `cannot free a borrowed pointer; only its owner frees it`, and refused a `return`, `cannot return a borrowed if result; move the payload or return an owning call`, since a returned borrow would launder into the caller's owner. An inferred bind records the verdict the branches agreed on, so `x := if c { vec_get(v, 0) } else { vec_get(v, 1) }` followed by `free(x)` is refused with no annotation written anywhere.

Escape joins the branches too. The value's frame view is the join of what the branches yield, so a branch that yields a view of the current frame makes the whole expression one, and storing it into a place that outlives the frame is refused exactly where that branch's value alone would be refused.

The statement `if` is unchanged. A one armed `if`, an `if` whose branches only act, and an `else if` chain in statement position each mean what they always have, and a branch that yields nothing is a question only in the expression form.

### The conditional operator

Added in 1.13.3. `c ? a : b` is that same selection with single expression branches: `c` is a `bool`, `a` is the value when it is true and `b` when it is false. It is the loosest operator on the ladder, looser than `||` and `&&` and tighter than assignment, so `x = c ? 1 : 2` assigns the whole selection and `a || b ? x : y` reads as `(a || b) ? x : y`. It is right associative, one of the two levels where the ladder's left association does not hold: `a ? b : c ? d : e` is `a ? b : (c ? d : e)`, so a chain reads as a sequence of tests taken in order with the last `:` carrying the fallback. Both branches are required, and a `?` whose first branch is not followed by a `:` is refused at the `?` itself, `a conditional needs a ':' and a second branch, as in c ? a : b`.

The parser builds the node the `if` expression builds, so the two spellings are one construct the whole way down. The branches agree on type by the same rule, the ownership verdict unifies across the two branches the same way, the escape of the value is the same join, and neither spelling accepts a shape the other refuses. `c ? a : b` is `if c { a } else { b }` with the braces taken off, and the `else` is written into the operator, so the missing `else` question cannot arise in this form.

The `:` does not collide with the two other colons the grammar spells. An annotation colon follows an identifier at a binding position, and a struct literal's field colon follows a field name inside the literal's braces; the conditional's `:` is read only after a `?` the parser is already inside, with the true branch already parsed, so no reading of a colon is ever in doubt. `x: int64 = c ? a : b` binds an annotated `int64` to a selection, and `P{ f: c ? a : b }` fills one field with one.

### Nesting and Chain Limits

A program's expression tree is bounded, and the bound is one budget of 500 units the parser spends. A nested block, a nested type, and a nested expression each cost a unit while the parser is inside them, and so does every link of a chain the grammar folds in a loop: a binary operator, a `|>` pipe, a postfix field, call, or index link, and a step of a `do` block, which desugars into one nested bind apiece. A chain reads flat and nests in the tree all the same, and resolve, the type pass, monomorphization, and code generation all descend that tree recursively, so the charge is what keeps a legal program from running a later pass off its stack. The practical ceiling is near 496 links at a statement position, which is far above written code; the longest chain in the compiler's own source is ten terms.

The refusal names the shape that spent the budget: `expression chain is too long; split it across bindings` for a folded operator chain, `field and call chain is too long; bind an intermediate value` for a postfix chain, `do block has too many steps; move some of them into a helper function` for a `do` block, and `expression nesting is too deep; simplify the expression`, `block nesting is too deep; simplify the function`, or `type nesting is too deep; simplify the type` for ordinary nesting. Each fix in the message is the real one: an intermediate binding costs nothing at runtime and lowers the tree. The guard reports once per file and the parse ends there, since a refused subtree consumes no tokens and every diagnostic after it would be an artifact of the refusal rather than a second problem in the source.

### Operators dusk does not have

A few operators common elsewhere are deliberately absent. Optional chaining `?.` and null coalescing `??` have no place in a language with no null: a managed pointer is single owner and every dereference is checked, and a missing value is `Maybe.None` or an `error`, which a `match` reads. The ternary `?:` sat on this list until 1.13.3, refused on the grounds that `if` already covers selection; it is spelled now, and it is that same `if`, one node under two spellings rather than a second selection with rules of its own. Spread `...` has no varargs to spread into. A concatenation operator, `<>` or a reused `++`, is also absent: `StringBuilder` owns string building explicitly, and a slice concatenation needs an allocator, which an operator has nowhere to name.
...
The ambient allocator's name is reserved for the whole function body. A binding that shadows a `using` parameter's name, in the body or in any nested block, is refused, `cannot shadow the allocator parameter '<name>'; alloc and free in this function resolve through it, so rename this binding`, since `alloc` and `free` resolve through that name and a rebinding would redirect them to an unrelated value, a fault when the shadow is not an allocator and a silently wrong arena when it is. The reject fires whatever the shadow's type, since the danger is the name, not the type it takes.

A method takes a `using` allocator exactly as a free function does, so bare `alloc` and `free` in a method body resolve through the method's `using` parameter. An `async func` takes a `using` allocator too. The async substrate re-materializes every parameter into a heap task frame a poll function reads across suspensions, and the ambient allocator lands there like any other parameter, so bare `alloc` inside an async body resolves the allocator by name through the frame and the passed arena backs the allocation rather than the default heap. The allocator survives a suspension: an `alloc` before an `await` and one after both draw from the same arena, since the poll re-enters at the await label and re-reads the allocator from its frame slot rather than losing it across the resume edge. A lambda body carries no ambient allocator of its own, so a binding of the reserved name inside a lambda is legal, and an `alloc` written in a lambda draws from the default heap; for the same reason a lambda cannot take a `using` parameter, `a lambda cannot take a using parameter; a lambda body has no ambient allocator, so allocate in the enclosing function or use the default heap`.
...
`free` follows ownership, and ownership follows the binding that received the allocation. A parameter borrows: the callee may read and write through it, but `free` on a parameter is refused with `cannot free a borrowed pointer; only its owner frees it`, and `move` on one is refused the same way, so a callee can never end a lifetime its caller still holds. A value read out of a container borrows too: `vec_get` hands back the stored pointer as a view of the container's backing, so freeing or moving that result is refused. The refusal is one model across every spelling. Freeing a container read directly, `free(vec_get(v, 0))`, is rejected with `cannot free a call result that may alias its arguments; vec_take or map_take removes an element as its owner`, the `move` form with the matching `cannot move a call result that may alias its arguments; vec_take or map_take removes an element as its owner`, and the method spelling `free(box.get(0))` rejects the same way, since a method's element read aliases its receiver exactly as a bare call aliases its argument. Binding the read to a name first and freeing the name is refused as a borrow all the same. `free` also validates the ground type of its operand: a `free` of a value that is not a heap pointer at all, an `int64` reached through a generic instantiation among them, is a type error, `free takes a heap pointer, not <ty>`, caught at check rather than handed to the allocator to fail inside `clang`. `move` is exempt, being an identity on a plain value.

A tuple literal member that names an owner hands the binder a borrow. `a, b := (owner, 1)` reads `owner` and leaves the ownership where it was, so `a` is a view of an allocation the source name still owns and still frees, and `free(a)` is refused, `cannot free a borrowed pointer; only its owner frees it`. The single binding refuses the same launder one step earlier, `b: *Box = owner` giving ``cannot copy an owning pointer; bind a `ref` alias or `move` it``, so the two spellings reach one model rather than leaving the tuple a way around it. This is the struct embed's rule spelled for a tuple: packing a live pointer into an aggregate is a read, not a transfer. `move(owner)` in the member position is the transfer, so `a, b := (move(owner), 1)` hands the binder the ownership and retires the old name, and a member that allocates directly, `fresh, k := (alloc(Box { n: 1 }), 5)`, is the binder's own the way any other `alloc` binding is. The record follows the value through a name and through nesting: `t := (keep, 1)` then `v, m := t` binds `v` as a borrow, and a launder through `((p, 1), 2)` survives the inner destructure at any depth. Before 1.13.2 only a member that already named a borrow was recorded, so a member naming an owner handed the binder the owner default and left one allocation with two names each claiming the right to free it, a double free the runtime aborts on rather than a diagnostic at the launder.

Inside a generic body a value whose type is a bare type parameter borrows. Once monomorphization instantiates the parameter with a managed pointer the caller owns, freeing or moving that value inside the callee would end a lifetime the caller still holds, so the surface pass treats a type parameter type as a borrow and refuses `free` and `move` on it with the same messages a concrete borrow gives, `cannot free a borrowed pointer; only its owner frees it` and `cannot move a borrowed pointer; only its owner can be moved`. The rule reaches every position a borrowed name can be introduced or carried: a `func f<T>(p: T)` parameter, a lambda parameter typed `T`, a plain binding or a rebind of such a value, a member destructured out of a tuple that a type parameter value was laundered through, the capture of a task lambda spawned over one, and the left side of a reassignment that would drop a value the binder owns at instantiation. A tuple binding records the borrow of its type parameter member, so a value laundered in through `pair := (x, 1)` and back out through `y, n := pair` is still a borrow at `y` and a `free(y)` is refused; a rebind of the tuple, `q := pair`, carries the borrow across the extra name. The reject is the concrete twin's exactly: the same `free` inside `func f(p: *Box)` already rejects, and the generic form now matches rather than admitting a double free the ground instantiation would only discover at runtime.

The tightening is one-sided on purpose, and the asymmetry is stated plainly. A bare type parameter binder is treated as a borrow for the restrictive obligations above, but it is never promoted to an owner, since marking a `T` binder an owner would false-reject a legal scalar copy at a non pointer instantiation, `dup(5)` and `n: int64 = take_id(41)` among them, where the value is an `int64` that copies freely and owns nothing. So a generic body may still copy and return a type parameter value as a borrow the caller keeps and frees once, and an `owning func take_id<T>(x: T) -> T` is an owner at a managed instantiation and inert at a scalar one. The durable close of the whole family is a ground ownership pass that judges each obligation on the substituted type after monomorphization; until it lands the borrow-side pessimism ships and the owner-side stays conservative.
...
The tree free contract is heap strings only. `free` of a string that is a literal rather than a heap allocation is a misuse the runtime names rather than a corruption it absorbs: the pointer lands inside the program image, the free path recognizes it, and the process aborts with `fatal: free of a string literal or other constant; it was never allocated`. A deep free is therefore safe only over a tree whose every string is heap allocated, which every `json_parse` result is, and a hand built tree carrying a literal string payload faults at that payload's `free`. The contract names the requirement rather than checking it at compile time.

The standard library ships this deep free as `std.json`'s `json_free(root)`, the worklist walk moved verbatim, the first standard library code to call the owning takes. It consumes `root`, freeing every node, string payload, object key, and backing buffer, so no pointer into the tree is valid afterward and a later dereference of any freed block faults named through the generational check. It requires a fully heap allocated tree, which every `json_parse` result is, and a hand built tree carrying a literal string payload faults by name at that payload's `free`, the same heap strings only contract above. A subtree reachable from two parents double frees, and the second free faults named rather than corrupting silently.
...
One limit is named rather than hidden. Over a value with no ownership to track, a scalar among them, the marker is inert: `ref n := 7` is accepted and records nothing, since a scalar copies freely and owns nothing to borrow from.

A destructuring binding takes the word with the meaning the single binding gives it. `ref a, b := pair` declares every name it binds a borrow of what the value already owns, so the source stays the single owner that frees, `free(a)` is refused with `cannot free a borrowed pointer; only its owner frees it`, and `move(a)` with `cannot move a borrowed pointer; only its owner can be moved`. The borrow follows the member paths down, so a binder that holds a nested tuple hands it to the names a second destructure takes out of that binder: after `ref inner, tag := ((owner, 1), 2)`, the `view, count := inner` that follows binds `view` as a borrow and refuses its free. A binder with no ownership to track stays inert exactly as `ref n := 7` does, so the scalar member of `ref view, tag := (owner, 1)` copies as it always did and only the managed binder records a borrow. Before 1.13.2 the word was accepted grammatically here and recorded nothing for either name, so a binder that inherited a member's owner record could free a pointer the source still owned and the launder reached the generational check at the later use rather than a diagnostic; it is a check error now.
...
`owning func` generalizes the take blessing into a declared marker any top level func can carry. A func head marked `owning` declares that its call result is the caller's own value, so the caller may `free` it or `move` it exactly as it may a `vec_take` result, and the checker blesses every call to the func the way it blesses the standard takes, exempting the result from the container alias condemnation. `owning` is a context sensitive modifier in the func head, positioned like `async`: `owning func take_slot(pool: *Vector<*int64>, i: int64) -> *int64`, and after `export` when both appear, `export owning func ...`. A lone `owning` not followed by `func` is a parse error, `expected 'func' after 'owning'`.

The marker is a top level func's alone, and it is scoped to a return the runtime can backstop. It is mutually exclusive with `async`, stacking the two a parse error, since after `async` the parser already requires `func`. `export "C" owning` is rejected at the head, `an owning func cannot be an exported C function`, since an exported C function has a C calling convention and no dusk ownership. A method cannot be owning, `a method cannot be owning; owning marks a top level func`, rejected in the impl parser, since the blessing keys on a bare identifier callee and a method call could never bless anyway. A `foreign` function never admits the word: a foreign return is a C value with no generation to check. `main` cannot be owning, `main cannot be owning; nothing calls main to take its result`, which also pins the newline trap, a stray `owning` on its own line would otherwise attach silently to the next func head.

An owning func hands over the one value it owns, so its return shape is checked at the declaration. A `void` return is rejected, `an owning func returns the one value it hands over; it cannot return void`, and a tuple return is rejected, `an owning func returns the one value it hands over; it returns one value, not a tuple`, since a tuple would leave the ownership of each element undeclared. This means a fallible owning take, one returning `(*T, error)`, cannot be spelled under the one value rule; a take that may fail probes its input first instead. A `*raw T` return is rejected too, `an owning func returns a managed value; a raw pointer carries no generation to check`, since the marker's whole guarantee is a value the generation check can judge and a raw pointer carries none. A bare generic return is legal, the `vec_take` shape, `owning func take_id<T>(x: T) -> T`: at a managed pointer instantiation the result classifies as an owner, and at a non pointer instantiation the marker is inert, since only a managed pointer result is ever an owner, so an `int64` result copies freely and never false rejects as an owning pointer.

The body owes nothing statically. `owning` is a declared contract exactly as the take mint is: the checker trusts the marker and verifies no provenance, so a func that marks itself owning and returns a borrow, a `vec_get` still living inside its container, is a lie the checker accepts. The generation backstop carries the lie to a named fault at the misuse site rather than a silent corruption: the caller frees the borrowed element, and the later read of the still live slot faults, `fatal: use of a freed or stale pointer at path:line`, exactly as a doubled take does. The honest boundary is the take's, stated once more and scoped to a managed return: the marker is a checked capability backed by the generation, not a proof the body was verified.

The standard library's `vec_take` and `map_take` carry the `owning` marker themselves, so they bless through the one declared mechanism every owning func uses. The name and shape table that once recognized the two takes by name is gone, and the blessing is the declared marker alone: a call owns its result because the func head said so, not because the callee's name matched a list.
...
A `match` used as an expression carries ownership the same way, judged across its arms with one voice. The result takes the type of the value its arms yield, a managed pointer result laid out as the fat pointer every carrier uses, so `x: *int64 = match *t { Leaf(q) => move(q), Empty => alloc(0) }` builds and extracts the owned payload where both spellings once died in codegen. The arms must agree on that type: every value yielding arm is checked, and an arm whose value disagrees in width or kind, an `int32` beside an `int64` or a pointer beside an integer, is rejected, `the arms of a match expression must agree on type`, so the single result slot codegen sizes from the first arm is one every arm fits and no value is silently truncated to it. That agreed type is the match's own type, so a match bound or passed against an annotation is width and type checked like any value, and a match used as another match's arm value carries its type into the outer agreement. A literal arm is ranged against that agreed type the same way, so an arm yielding `300` beside an arm yielding an `int8` is refused, `literal 300 does not fit in 8 bits`; before 1.13.3 an inferred bind of such a match stored the literal into the arm slot truncated and the program printed the low bits, 44 for the 300 the source wrote in full. An arm body may end in a bare `match`, which value-izes into the enclosing match: the last statement of a braced arm body, when it is a `match`, becomes the arm's yielded value, so the inner match's arms supply what the outer arm hands up and no name is bound to carry it. Since 1.13.3 an arm body may end in an `if` the same way: a tail `if` carrying an `else` is an expression and value-izes into the enclosing match exactly as a tail `match` does, its branches supplying what the arm hands up, while an `if` with no `else` stays the statement it is everywhere, so an arm ending in one yields nothing and is refused by the rule below. This nests to any depth, and the type agreement and the ownership verdict penetrate every level, the inner match's agreed type carried into the outer agreement and the inner match's ownership judgment read as the arm's own rather than assumed. Every arm must yield a value this way or diverge: an arm whose block ends in a statement that stores nothing, an `if` with no `else` or a `while`, and does not `return`, `break`, or `continue` out of the match is rejected, `every arm of a match expression must yield a value or return`, which closes a hole where such an arm left the single result slot uninitialized and the outer bind read whatever the slot happened to hold. The arms must agree on the ownership of the result too: the result is an owner only when every arm yields an owner, a `move` of a payload binder, an `alloc`, an arm local fresh owner, or an owning call among them, and a borrow only when every arm yields a borrow. An arm that yields a bare owner binder without moving it is the copy of an owner and is rejected, ``cannot copy an owning pointer; bind a `ref` alias or `move` it``, the same message the statement position gives; an arm that hands out a fresh owner it allocated inside the arm is a move out, not a copy, and keeps its owner. Arms that disagree, an owning arm beside a borrowing one, are rejected at the odd arm, `the arms of a match expression must agree on ownership of its result`. A `match` over a borrowed subject yields a borrow through every spelling, so extracting a payload from it and freeing the result is refused as a borrowed free, and returning that result is refused the same way, `cannot return a borrowed match result; move the payload or return an owning call`, since a returned borrow would launder into the caller's owner. This verdict is recorded even when the bind carries no annotation: an `x := match *t { ... }` whose arms borrow records the borrow under `x` exactly as an annotated managed bind does, so a later `free(x)` is refused, `cannot free a borrowed pointer; only its owner frees it`, where without the recorded verdict the inferred binding's surface wildcard would hide the managed result and the borrow would launder into a free of a pointer the subject still owns. An arm cannot yield a lambda literal, `a match arm cannot yield a lambda; bind the function to a name outside the match`, since the arm's fat result slot has no shape for a closure value.

Some gaps remain, and they are named rather than hidden. An unannotated bind of a container read, `q := vec_get(v, 0); free(q)`, passes the check and faults at runtime: a bare generic return is not substituted on the surface pass where ownership runs, so the read is never classified as managed and the later `free` is not caught until the generation check aborts it. A container read through an opaque callee, a lambda value or an interface method dispatched at runtime, passes the check the same way, since the checker cannot see across the call to know the result aliases an argument. The name and shape blessing of a user function named `vec_take` or `map_take` is gone: blessing is the declared `owning` marker alone, so a user func of either name that is not marked `owning` no longer owns its result, and a direct free of a container read inside such a call is rejected exactly as any other unmarked func's is. A program that leaned on the old by name recognition of its own `vec_take` marks the func `owning` to restore it; the flip is raise-only, never a silent loosening. A user `owning func` named after a builtin, `move` for one, is accepted and contributes a blessing but is unreachable, the builtin winning dispatch, a pre-existing reserved name class the checker admits and nothing can call. Freeing the result of a wrapper that returns a fresh box directly is rejected conservatively, where binding it to a name and freeing the name is accepted. Every one of these is a leak or a named runtime fault, never a silent corruption.
...
A `defer` reads the names in its deferred expression when it runs, at a function return. A name the deferred expression binds within itself, a `match` arm binder inside the deferred expression among them, resolves normally in that expression's own scopes; a name that reaches outward to the enclosing function resolves in the function frame the `defer` was registered in, not in whatever nested block is open at the return site. Because registration is confined to the top level of the function body, that frame is exactly the environment the `defer` was written against. A `defer println(x)` registered while `x` is the outer binding prints the outer value even when a nested block has shadowed `x` and returns from inside that shadow, so the block prints its own binding and the deferred read prints the frame's.
...
A `free` of a pointer dusk never allocated is undefined and goes unnamed. A C `malloc` result that crossed back into dusk as a `*raw T` carries none of the header the heap writes in front of a payload, and the runtime has no way to tell such an address from one of its own, so freeing one reads a size and a generation out of whatever bytes sit before it. Memory that came from the other side of the foreign boundary is released by the side that allocated it.

A second `free` of one block is caught in the allocator too, and aborts `fatal: double free` with no source location, since the allocator knows the block and not the line that gave it up. A managed pointer rarely reaches it: the generational check catches that case earlier and more precisely, `fatal: use of a freed or stale pointer at path:line`, because the pointer's generation no longer matches the block's. The allocator's own guard is what covers the layer beneath, a raw or untracked block with no generation to compare. Since 1.15.1 it covers every parked block, the freed state being a bit in the block's own header, where the older guard remembered a fixed window of parked blocks and let a second free of anything past that window through without a word.

One more misuse is caught inside the allocator rather than at a codegen emitted check, so it aborts with a message and no location the way the async family does. A string literal, and every other constant the compiler parks in the program image, carries none of the allocation header the generational heap puts in front of a payload, so `free` on one used to read a size and bump a generation in read only memory and die by signal with nothing named. Since 1.14.1 the runtime tests the pointer against the loaded image, `[__executable_start, _end)`, before it touches the block, and a hit aborts with `fatal: free of a string literal or other constant; it was never allocated`. Neither the heap nor an mmap region ever falls inside that range, so a real allocation never trips it. The bounds are linker defined symbols, so the check is an ELF platform check: on a target that does not define them, wasm and Mach-O today, it compiles out and a free of a constant keeps its older undefined behavior. The range named is the executable's own, so a constant sitting in the read only data of a shared library the program loaded falls outside it and freeing one still dies by signal.
...
An interface value is a fat pointer, a data pointer paired with a vtable pointer, and boxing a concrete struct into one works correctly when the interface value sits inside a struct field, an enum variant's payload, or an array element: the struct literal, the enum constructor, and the array literal each box the concrete value into the field, payload, or element's fat pointer as they build it, so a later method call dispatches through the stored interface exactly as it would through a bare interface binding. Boxing a struct to an interface does not work at every position, though. Returning an interface value by value is rejected outright, since the boxed payload would sit in a frame slot that dangles the moment the function returns: `returning an interface value is not supported; return the concrete type or a pointer to it`. The reject reads the declared return type and refuses an interface value embedded at any depth, not only a bare one: an interface member of a returned tuple, a struct field or an embedded field that resolves to one, an enum variant payload, an array element, and the pointee of a returned pointer are each caught, since every one hands back a fat interface value whose data pointer still names the returning frame's boxed concrete value. A function returning `(Shape, int64)` or `*Shape` from a frame local dangles exactly as a bare `-> Shape` does and is refused with the same `returning an interface value is not supported; return the concrete type or a pointer to it`. This closes a hole a related repair opened: once a tuple carrying an interface member could be built rather than dying in `clang`, an interface returned inside one read a dead frame slot at runtime, so the return walk now sees the carrier and rejects it at check. An interface value inside a tuple passed as an argument, distinct from a return, is rejected where it is written rather than accepted and miscompiled: `an interface value inside a tuple is not supported; return or pass the concrete type, or box it outside the tuple`.
...
**Cost and collection.** A mint is one allocation on the collected heap. Collection is amortized: it runs automatically once the byte debt since the last collection crosses a threshold that doubles with the live set, at whichever mint trips it, and a program can force one directly through `gc_collect`. The scan is conservative: every word on the anchor thread's stack between a collection point and the thread's high water mark, every collected reference reachable through the generational heap's own live registry, and every root region the async substrate registers for a task frame or a closure environment, is read as a possible pointer and, where it lands inside a live collected block, keeps that block and everything it in turn reaches. A conservative scan only over retains: a stray word that merely looks like a pointer keeps a block alive one collection longer than it needed to, never the reverse. This is mark and sweep, not moving: a collected block never relocates, so a raw address into one stays valid across a collection for as long as the block itself stays live. A precise, moving collector is not this one. The scan does not depend on the optimization level the program was built at. It brackets the anchor thread's real stack base, recorded when the collector anchors rather than inferred from the frame layout an unoptimized build happens to produce, and it spills the callee saved registers into the scanned region before it walks, so a pointer a register allocator kept in a register instead of a stack home is still read as a root. A `dusk build --release` therefore collects exactly as a plain build does.
...
A generic instantiates over concrete types only. An interface cannot be a generic type argument: `Box<Speaker>` is rejected with `an interface cannot be a generic type argument; generics are monomorphized over concrete types`, since monomorphization stamps one copy per ground type and an interface names a set of types, not one. A generic that should range over an interface's implementors takes the interface as a plain parameter or field type instead, where the value carries its vtable. The reject reads the type argument as written: a bare interface name is refused, and an interface behind a constructor is a carrier of interface values and stays legal, so `Box<Speaker[]>`, `Box<Speaker[2]>`, and `Box<*Speaker>` are each accepted, at any nesting depth and through a generic field of another generic, since every element there is already the two word fat value dispatch expects. The ordinary conversion rules still apply inside the carrier: a slice of concrete structs handed to a `Box<Speaker[]>` is refused exactly as the non generic field case is, `cannot pass a slice of 'Dog' as a slice of interface 'Speaker'; a slice of concrete values cannot be reinterpreted as a slice of interfaces`, since one word of payload cannot be read as a two word fat value.

A type parameter in scope names that parameter wherever the body spells it, a value position included: `sizeof(K)` inside a body generic over `K` reads the type parameter, never a variant some enum elsewhere in the program happens to spell `K`, since the parameters a signature declares are consulted before any variant table. The reading matters most across an import boundary, where the other way round an importer's own enum would decide how library source it never wrote reads its own type names.

An argument is checked against its parameter's type by every spelling it can take, a named local, a literal, a call result, and a struct field access alike. A field of a concrete struct resolves to its declared type and checks there: `str_eq(n.name, "x")` where `name` is an `int64` field is rejected `argument N has the wrong type` at the call, not left untyped to surface later as an IR type error in `clang`. A field whose base is generic or otherwise unresolved stays permissive.

A builtin call name is reserved. A function cannot be declared with the name of a builtin the checker and code generator resolve first, `move`, `alloc`, `free`, `sizeof`, `hash`, the print family, and the functional builtins among them, since the builtin wins the call and the declared body would be silently dead code, a call to it reaching the builtin rather than the function. A top level declaration of such a name is refused, `'<name>' is a builtin; a function cannot take its name`. A binding of such a name is refused only where it could actually capture a call: a function typed binding anywhere, a parameter or lambda parameter whose annotated type is a function or a bare type parameter, each with `'<name>' is a builtin; a binding cannot take its name`. A binding whose type provably is not a function, a `filter: string` local or a `map: int64` parameter, stays legal, since a call spelling the name still reaches the builtin and the binding shadows nothing. A parameter reject points at the function head, the nearest span a parameter name carries. The rule is one coherent reservation across the whole builtin call table, closing a class where a user `move` was silently dead, a user `println` silently swallowed its own output, and an owning `alloc` split at `clang`; it is raise-only, refusing a declaration that never had a working meaning. This is stricter than the primitive type name reservation the numeric casts carry, `int32` and its siblings, which collide only with a function declaration and stay legal as a variable or a field name.
...
The copy is one per lambda site, not one per execution, and inside a loop that distinction is visible. A synchronous lambda's environment is a single piece of storage belonging to the place the lambda is written, so a lambda created inside a loop body writes its captures into that same storage on every iteration. A lambda called during the iteration that created it reads that iteration's values, which is what a body normally wants. A closure stored somewhere that outlives the loop, into a vector, a field, or a channel, reads the captures of the last iteration that ran, since every iteration wrote over the one environment; a program that needs one closure per iteration cannot express it through capture alone and passes the value as an argument instead. This is the rule for every loop the language has, a `while`, a `for` over an array, a slice, a string, or a container, and a counted `for` alike, and it is a property of the lambda environment rather than of any loop form. A lambda written inside an `async func` is the exception: its environment is built fresh each time the lambda is created, so closures made across the iterations of a loop there keep their own captures.

A lambda literal cannot be called where it is written. The immediately invoked form, `lambda (x: int64) -> int64 { return x + 1 }(5)`, and its parenthesized twin, `(lambda (x: int64) -> int64 { ... })(5)`, are both refused, `a lambda literal cannot be called where it is written; bind it to a name and call the name`, since the checker lowers a lambda through the name it is bound to and a literal in call position has no name to bind it against. Bind it first, `f := lambda (x: int64) -> int64 { return x + 1 }; r := f(5)`, and call the name. This is the reason an ambient allocator or a scoped binding cannot be threaded through such a call directly either; the enclosing function does the allocation or the binding stands on its own.
...
A method signature may name a generic instance. `boxed() -> Box<int64>` and `take(b: Box<int64>) -> int64` are both legal in an interface declaration, and monomorphization grounds the declaration in place, so the vtable thunk, the slot that receives a dispatched call, and every impl body agree on one mangled type. Before 1.13.2 the declaration kept its type as written while the impls were ground to the mangled form, so a program with such a signature checked clean and then died in `clang` on the type disagreement.

An interface declaration takes no type parameters of its own. `interface Speaker<T>` is rejected, `the generic interface 'Speaker' is not supported yet; spell its methods over concrete types, since an interface value erases the type it holds and cannot be monomorphized`. An interface value is a data pointer paired with a vtable pointer and erases the concrete type behind them, so no dispatch site can pin `T`, and every impl shares one vtable slot whose signature has to be a single concrete type. The declaration was previously parsed and treated as though its parameter were `int64`, building a thunk that disagreed with any impl spelling another type. Spell the method over the concrete types instead, or take the varying part as a type parameter of a generic function that holds the interface value.

An enum value cannot stand where an interface is expected. Handing one to an interface parameter, binding one to an interface annotation, and returning one as an interface are each rejected, `cannot use the enum 'Opt' as interface 'Sized'; only a struct can implement an interface, so wrap the enum in one`, which names the fix. An interface value is a data pointer paired with a vtable pointer, only a struct can carry the `impl` that fills the vtable, and an enum named as an impl's receiver is refused where it is written, so an enum reaching an interface slot has nothing to dispatch through. Before 1.13.3 the pairing went unjudged, since the interface erases to a wildcard every ordinary comparison agrees with, and the value reached code generation where the dispatched call had no vtable to read.
...
A field read answers with the field's declared type wherever the base names a non generic struct, through a dereference too, so `(*p).x` is the `float64` the declaration spells. A field the struct does not declare is refused, `'S' has no field 'nofield'`, the message a struct literal naming an unknown field already gave and now the read's as well, rather than left a hole that surfaces later; a field holding a generic container carries its type parameter with it, so `vec_len(self.items)` inside an `impl` infers the element type from the field alone with no rebinding; and a field is a place of its declared type at every operator that writes one, so `p.f++` on a `float64` field is refused exactly as any other float place is. A field is read through a pointer with an explicit dereference: `p.n` where `p` is a `*S` is refused, `a field read through a pointer needs an explicit dereference; write (*p).field`, the read's half of the rule the store form already carried, and `(*p).n` is the spelling that lowers.
...
`vec_fold(v, init, f)` reduces the vector to a single value, folding `f` left to right over the elements from `init`, the accumulator its first argument and the current element its second, each call's result the next accumulator and the final one returned. An empty vector returns `init` with `f` uncalled, and the source is neither mutated nor freed. The accumulator type is independent of the element type, so a `Vector<T>` folds into a value of another type, a running sum, a joined string, or a built structure.
...
| uint8     | uint8(v) -> uint8                       | numeric cast to uint8                                             |
| uint16    | uint16(v) -> uint16                     | numeric cast to uint16                                            |
| uint32    | uint32(v) -> uint32                     | numeric cast to uint32                                            |
| uint64    | uint64(v) -> uint64                     | numeric cast to uint64                                            |
...
`sizeof` takes a named type or a value, so `sizeof(int64)` and `sizeof(x)` are both legal, but an inline tuple type is not a spelling it accepts: `sizeof((int64, int32))` is refused, `sizeof takes a type or a value; a tuple type is written through a named struct`, since a tuple type written inline has no named layout to size and would otherwise be misread as a value expression. Wrap the members in a named struct and take the size of that. The reject reads the argument as a type spelling only when it names no local, so `sizeof(t)` over a tuple valued binding `t` sizes the value as usual.
...
Added in 1.2.0 as integer width casts, widened in 1.5.0 to the whole numeric set and in 1.13.0 to the unsigned widths. `int8(v)`, `int16(v)`, `int32(v)`, `int64(v)`, `uint8(v)`, `uint16(v)`, `uint32(v)`, `uint64(v)`, `char(v)`, `rune(v)`, `float32(v)`, and `float64(v)` convert a scalar value explicitly to the named type. The source may be any integer family value, an `int` or a `uint` of any width, a `char`, a `rune`, or a `bool`, or a `float32` or `float64`. The conversion follows the operand and target kinds:

- integer to integer: two's complement truncation going down, extension going up by the **source** type's signedness, so a signed source sign extends and an unsigned source, a `char`, or a `bool` extends by magnitude. A cast between two integers of the same width moves no bits at all and is a reinterpretation, so `uint8(-1)` is `255` and `int8(255u8)` is `-1`.
- integer to float: the integer's value as a float, exactly when it fits the significand, an unsigned source, a `char`, and a `bool` read as a magnitude. A `uint64` past the significand's reach rounds, so `float64(18446744073709551615u64)` prints `1.84467e+19`.
- float to integer: truncation toward zero, so `3.9` casts to `3` and `-2.9` to `-2`. A magnitude beyond the target integer's range saturates to its nearest bound rather than wrapping or leaving the result undefined, and a NaN casts to zero, so a misused cast stays deterministic instead of undefined the way C's own conversion is. The bounds are the target's own: a signed width saturates into `[-2^(w-1), 2^(w-1)-1]` and an unsigned width into `[0, 2^w - 1]`, so a negative float to any unsigned width lands on `0`.
...
`for x in xs` takes an array, a slice, or a string. Over a string it iterates the bytes as `char`, front to back, with the string's length read once by a NUL scan at loop entry rather than rechecked each iteration. A source with no element type is rejected at check, `cannot iterate <type>; a for loop takes an array, a slice, a string, a *Vector, or a *Map`, rather than being accepted and failing to link. A `for` whose head reads a range counts rather than iterating a value at all; see [The counted loop](#the-counted-loop).

Added in 1.11.0, a `for` also iterates the two standard containers when the iterand is a pointer to one. `for x in v` over a `*Vector<T>` binds each element in index order, and `for k in m` over a `*Map<K, V>` binds each key in insertion order, the map's documented order. The pointer is required: iterating a container by value is rejected, `iterate the container through a pointer, as in v: *Vector<T>`, because the loop re-reads the container's length and each element every iteration through the fat pointer. A container loop is not a builtin. The checker classifies the iterand on its type and the loop lowers, before monomorphization, into the exact index loop a program would write by hand against `vec_len` and `vec_get`, or `map_len` and the positional key accessor, so container layout stays in the standard library and codegen never learns it. Because the length re-reads every iteration, growing the container inside the body is visited by a later iteration and shrinking it terminates early; freeing the container inside the body does not read a stale snapshot but faults at the next iteration's generation check.

The map pair form `for k, v in m` binds a key and its value together and is map only; two binders over any other iterand is rejected, `two loop names bind a map's key and value; this iterand has only elements`. The binder of a container loop is a borrow the container still owns, the same as the result of `vec_get` or `map_key_at`: mutating through a managed element is legal, but freeing or moving the binder is rejected, `cannot free a borrowed pointer; only its owner frees it`. To free an element, drain it with `vec_take` or `map_take`, which hand over the owner; `vec_take` inside a forward loop shifts the survivors down, so the loop's next index skips the shifted successor, the drain from the back idiom `while vec_len(v) > 0 { vec_take(v, vec_len(v) - 1) }` avoids it.
...
Added in 1.16.0, a `for` also counts. `for i in lo..hi { ... }` runs its body with the binder at `lo`, then `lo + 1`, and on up to but not including `hi`, which is what the half open range `lo..hi` already means everywhere else in the language: the bounds of a counted loop and the bounds of the range slice `xs[lo..hi]` are read the same way, so `for i in 0..xs.len` walks exactly the indices `xs[0..xs.len]` views. A `hi` at or below `lo` runs the body zero times rather than faulting or wrapping around.

```text
for i in 0..5 { println(i) }            // 0 1 2 3 4
for i in 0..0 { println(i) }            // no iterations at all
for i in 10..0 by -2 { println(i) }     // 10 8 6 4 2
```

`by` names the step. `for i in lo..hi by step` moves the binder by `step` where the plain form moves it by one, and `step` is written as an integer literal, never zero. A positive step counts up and the loop runs while the binder is below `hi`; a negative step counts down and the loop runs while the binder is above `hi`, so a descending loop is written with `lo` above `hi` and `for i in 0..10 by -1` runs the body zero times instead of forever. The step is a literal rather than an expression because its sign decides which comparison the loop is built around, a question the compiler answers once rather than the program answering it every iteration. A step that is not one is refused, `the step after 'by' must be an integer literal, as in for i in 0..n by 2`, and a zero step is refused with it, `a step of 0 never advances the loop; use a nonzero step`, since a loop stepping by nothing either never reaches `hi` or never runs at all and neither is a thing a program means to write.

`by` is a contextual word, not a reserved one. It is read as a keyword only where a `for` head has just finished reading a range, and everywhere else in the language it is an ordinary identifier, so a binding, a parameter, a struct field, and a function each named `by` keep their spelling and no program written before this release loses a name to the new form. Written in a `for` head after anything but a range it is refused rather than read as a name, `'by' steps a counted range; write for i in lo..hi by 2`, since a step has nothing to say to a container, an array, a slice, or a string.

A counted head is classified by its syntax, not by a type. A `for` whose iterand is a range is a counted loop from the moment the parser reads the `..`, so the type based classification that sorts an array from a slice from a container never sees a range and the `cannot iterate <type>` message keeps the list it has. One consequence is worth naming: the two binder form belongs to a map and nothing else, so `for i, j in 0..n` is refused, `two loop names bind a map's key and value; a range yields one integer`.

Both bounds are evaluated once, at loop entry, left before right. A call in a bound runs one time however many iterations follow it, and assigning to a name a bound read does not move a trip count the loop has already fixed.

A bound is `int64`, and only `int64`. A bound of a type that is not an integer at all is refused naming the type it found, `a range bound must be an integer, not float64`, and a range written as a bound of another range, `for i in 0..(1..2)`, answers the same way, `a range bound must be an integer, not a range`. An integer of another width or another signedness is refused too, `a range bound must be int64, not uint32; cast the bound, as in int64(n)`, and the cast in the message is the whole fix. A `char` and a `rune` stand in for an integer in plenty of other positions and are refused here with that same cast message naming the type, `a range bound must be int64, not char; cast the bound, as in int64(n)`, whether the bound is written as one directly, `for i in 0..'A'`, or arrives as one through a generic call the head cannot see into. The head and the binding the rewrite produces are judged by the one rule, so the backstop that catches a bound on the ground pass names the bound with this message rather than reporting a type disagreement against a synthesized name a program never wrote. This is deliberately stricter than the range of a slice index, which accepts what it can widen: a counted loop is rewritten into source the program did not write, so a conversion the compiler inserted there would be invisible at the head that asked for it, and a cast across the signed and unsigned families saturates, which would quietly turn a bound into a different number than the one spelled. A bare literal bound adapts to `int64` the way a literal adapts at any other binding position, so `for i in 0..10` needs no suffix and no cast. Inside a generic body a bound whose type is still a bare type parameter is refused rather than waiting for monomorphization, `a range bound must be int64; a value of type parameter T is not known to be one`, since the rewrite runs before the type is ground.

The binder is a fresh immutable `int64` each iteration, the discipline every other `for` binder carries. Assigning to it is refused by the ordinary immutability rule, `cannot assign to immutable 'i'`, so the loop's own step is the only thing that moves it and a body that wants a different number computes one into a binding of its own. The binder takes no type annotation and is always `int64`. A lambda written in the body captures the binder by the rule [Anonymous Functions (Lambdas)](#anonymous-functions-lambdas) states, one environment per lambda site, so a lambda called inside its own iteration reads that iteration's count while a closure stored past the loop reads the last iteration's, exactly as in every other loop the language has.

`break` and `continue` mean here what they mean in every other loop. The step runs before the body, so `continue`, which jumps to the loop's condition, lands on a count that has already advanced and a loop that skips terminates instead of spinning on the same number, and `break` leaves immediately.

A counted loop is not a builtin and code generation never learns it. The checker classifies the loop on its head, and the same pass that rewrites a container loop before monomorphization rewrites this one into the `while` a program would otherwise write by hand: a hidden counter `__ri$N` initialized to `lo`, a hidden bound `__rhi$N` holding the once evaluated `hi`, both annotated `int64` so the ground pass that runs after monomorphization has a declared type to check them against rather than a hole to infer, a comparison of the counter against the bound in the direction the step's sign chose, then the binder bound off the counter, the step, and the body, all of it spanned to the `for` header. The rewrite first appears in the `mono` dump, and the `parse` and `desugar` dumps carry the loop exactly as it was written.

The counter is an `int64` and wraps at the edge of that width like any other `int64` arithmetic in dusk, with no guard of its own. A loop whose bound and step carry the count past the maximum wraps to the minimum and keeps going, which is exactly what the hand written `while` it replaces did; the rewrite adds no check the program would not have had.

There is no inclusive counted loop. `..=` is a slice index form, described in [Inclusive range](#inclusive-range), and a `for` head takes the half open range only: `for i in 0..=n` is refused with the rewrite spelled out, `a counted loop takes a half open range; write lo..hi + 1 instead of lo..=hi`. One spelling of a range in a loop head is worth more than a second one that differs from it by a single character and by one last iteration.
