...
A method takes a `using` allocator exactly as a free function does, so bare `alloc` and `free` in a method body resolve through the method's `using` parameter. An `async func` cannot take a `using` parameter yet, `an async func cannot take a using parameter yet; allocate before the call or use the default heap`, since the async substrate re-materializes parameters into a heap task frame a poll function reads across suspensions and dispatching an ambient allocator through that frame is not wired. A lambda body carries no ambient allocator of its own, so a binding of the reserved name inside a lambda is legal, and an `alloc` written in a lambda draws from the default heap; for the same reason a lambda cannot take a `using` parameter, `a lambda cannot take a using parameter; a lambda body has no ambient allocator, so allocate in the enclosing function or use the default heap`.
