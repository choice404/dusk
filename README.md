...
0.9.3. The compiler runs the whole pipeline. It lexes, parses, resolves names, type checks, monomorphizes, and emits code, backed by a golden and unit test suite. The standard library and the multi module sample both build and run. The language surface is frozen for the bootstrap as of 0.5.4: the releases from 0.6.x through 0.9.x rewrite the compiler in dusk without changing the language, so a program that compiles today keeps compiling across that line, and new surface resumes only after 1.0.0.

The bootstrap's codegen line closes with 0.9.3: dusk1, the compiler written in dusk itself, now lowers every construct the language surface carries, `async func`, `await`, and `async_run` included, to LLVM IR byte identical to the Rust compiler's own output, and builds its own roughly 25,000 line source under itself. The releases from 0.9.0 through 0.9.3 carried that line one layer at a time, the scalar spine, the aggregates, closures and the collector, and now async, each one gated against the full example corpus before the next began.
