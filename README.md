...
0.9.4. The compiler runs the whole pipeline. It lexes, parses, resolves names, type checks, monomorphizes, and emits code, backed by a golden and unit test suite. The standard library and the multi module sample both build and run. The language surface is frozen for the bootstrap as of 0.5.4: the releases from 0.6.x through 0.9.x rewrite the compiler in dusk without changing the language, so a program that compiles today keeps compiling across that line, and new surface resumes only after 1.0.0.

The bootstrap closes with 0.9.4: dusk1, the compiler written in dusk itself, builds its own roughly 25,000 line source into a second dusk1, that second one builds it again into a third, and all three stages land on byte identical binaries and byte identical compiler IR, the fixpoint a self hosting compiler eventually has to reach. The releases from 0.9.0 through 0.9.3 carried codegen one layer at a time, the scalar spine, the aggregates, closures and the collector, and async, each one gated against the full example corpus before the next began; 0.9.4 is the release that climbs the stage ladder itself to the top rather than adding another layer beneath it. The bootstrap arc that started at 0.6.0 is done. What comes next lives in the 1.0.0 line.
